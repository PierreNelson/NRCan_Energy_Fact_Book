...
class Section1Indicators(SectionProcessor):
    ...
    def _process_ghg_emissions(self) -> int:
        """Process GHG emissions by economic sector from ECCC CSV (Page 20 / Page 132)."""
        try:
            source_cfg = (
                self.config.sections.get("section1_indicators", {})
                .get("sources", {})
                .get("ghg_emissions", {})
            )
            ...
            data_rows, metadata_rows = build_ghg_emissions_rows(source_urls=source_urls)
            if not data_rows:
                return 0
...
# =============================================================================
# GHG emissions (Page 20 / Page 132) — ECCC GHG_Econ_Can_Prov_Terr.csv
# =============================================================================
...
def _fetch_ghg_econ_csv(source_urls: Optional[List[str]] = None, timeout: int = 120) -> Tuple[bytes, str]:
    """Download GHG economic-sector CSV; tries primary then fallback URLs."""
    urls = source_urls or GHG_DEFAULT_SOURCE_URLS
    last_error: Optional[Exception] = None
    for url in urls:
        try:
            response = requests.get(url, timeout=timeout, headers=GHG_REQUEST_HEADERS)
            response.raise_for_status()
...
def _fetch_ghg_crude_production_pct_change(base_year: int, end_year: int) -> Optional[int]:
    ...
    try:
        from .section6_oil_gas import build_oil_sands_rows
...
def build_ghg_emissions_rows(
    ...
    content, source_url = _fetch_ghg_econ_csv(source_urls)
    ...
    for source_name, vector in GHG_SOURCE_TO_VECTOR.items():
        found = sum(1 for (s, y) in by_source_year if s == source_name and y >= chart_min_year)
        if found:
            print(f"  Found '{source_name}' -> {vector} ({found} chart years)")
