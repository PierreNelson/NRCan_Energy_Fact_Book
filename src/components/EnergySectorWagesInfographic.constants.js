...
export const exportWagesInfographicPng = async (figureEl, { scale = 2 } = {}) => {
    ...
    const exportRoot = figureEl.closest('.energy-sector-wages-inner');
    const titleEl = exportRoot?.querySelector('#energy-sector-wages-title');
    const layout = figureEl.querySelector('.energy-sector-wages-infographic-layout');
    ...
    const drawBulletMarker = (li) => {
        const liBox = rel(li);
        const textEl = li.querySelector('.energy-sector-wages-bullet-text');
        if (!textEl) return;
    ...
    const drawDomWords = (el) => {
        ...
        const walk = (node, inherited) => {
            ...
            const accent = inherited.accent || node.classList?.contains('energy-sector-wages-accent');
    ...
    const bgLoaded = await waitForImage(layout.querySelector('.energy-sector-wages-bg'));
    if (bgLoaded) {
    ...
    layout.querySelectorAll('.energy-sector-wages-bullet-item').forEach((li) => {
        drawBulletMarker(li);
        const textEl = li.querySelector('.energy-sector-wages-bullet-text');
