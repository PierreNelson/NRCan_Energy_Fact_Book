...
/** Label row Y anchors in viewBox coordinates (from page111_bg.svg reference artwork). */
export const LABEL_SLOT_TOP_Y = 79;
export const LABEL_SLOT_BOTTOM_Y = 247;

/** Barrel interior geometry in viewBox coordinates (from page111_bg.svg). */
...
export const exportCanadianProductionInfographicPng = async (figureEl, { scale = 2 } = {}) => {
    ...
    const artEl = wrapper.querySelector('.page111-infographic-art') ?? wrapper;
    ...
    const svgEl = artEl.querySelector('.page111-barrel-svg');
    if (svgEl) {
    ...
    const drawLabelRow = (rowEl) => {
        ...
        const pctEl = rowEl.querySelector('.page111-label-pct');
        if (pctEl) {
        ...
        const dotEl = rowEl.querySelector('.page111-label-dot');
        if (dotEl) {
        ...
        const nameEl = rowEl.querySelector('.page111-label-name');
        if (nameEl) {
    ...
    figureEl.querySelectorAll('.page111-label-row').forEach(drawLabelRow);
