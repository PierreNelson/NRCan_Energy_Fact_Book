...
/** Full composite with layers overlapping like energy_sector_demographics.png (not stacked). */
...
/** Overlay anchors (% of layout strip). Calibrated against energy_sector_demographics_bg_2.png. */
...
export const exportDemographicsInfographicPng = async (figureEl, { title, scale = 2 }) => {
    ...
    const exportRoot = figureEl.closest('.energy-sector-demographics-inner');
    const titleEl = exportRoot?.querySelector('#energy-sector-demographics-title');
    const wrapper = figureEl.querySelector('.energy-sector-demographics-infographic-wrapper');
    ...
    const drawRichText = (el) => {
        ...
        el.childNodes.forEach((node) => {
            if (node.nodeType === Node.TEXT_NODE) {
                ...
                const accent = node.classList?.contains('energy-sector-demographics-accent');
                text.split(' ').filter(Boolean).forEach((word) => {
    ...
    const bgBack = wrapper.querySelector('.energy-sector-demographics-bg-back');
    const bgFront = wrapper.querySelector('.energy-sector-demographics-bg-front');
    ...
    wrapper.querySelectorAll('.energy-sector-demographics-bubble-text').forEach(drawRichText);
