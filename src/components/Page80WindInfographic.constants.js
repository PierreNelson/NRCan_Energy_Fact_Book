...
/**
 * Overlay slots per metric on page80_bg.png.
...
export const getGrowthOverlayParts = (growthKey, getText, lang) => {
    if (growthKey === 'more_than_tripled') {
        return {
            prefix: getText('page80_growth_prefix_more_than', lang),
            word: getText('page80_growth_tripled', lang),
        };
    }
    return {
        prefix: '',
        word: getText(`page80_growth_${growthKey}`, lang),
    };
};
...
export const exportPage80InfographicPng = async (figureEl, { scale = 2 }) => {
    ...
    const bgImg = wrapper.querySelector('.page80-bg-image');
    const overlayEls = [...wrapper.querySelectorAll('.page80-overlay')];
