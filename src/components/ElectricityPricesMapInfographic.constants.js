/**
 * bar + icon layout tuning (compare to electricity_prices.png).
 *
 * MANUAL TUNING GUIDE
 * -------------------
 ...
 */
import electricityPricesBgIcons from '../assets/electricity_prices_bg_1.png';
import electricityPricesBgMap from '../assets/electricity_prices_bg_2.png';

export { electricityPricesBgIcons, electricityPricesBgMap };
...
/** Native map aspect ratio (electricity_prices_bg_2.png). */
...
/** Native aspect ratio of electricity_prices_bg_1.png (width / height). */
export const electricity_prices_ICONS_ASPECT = 1912 / 823;
...
export const exportElectricityPricesChartPng = async (wrapperEl, plotlyImgDataUrl, { scale = 2 } = {}) => {
    ...
    const mapImg = wrapperEl.querySelector('.electricity-prices-map-bg-image');
    if (mapImg?.complete && mapImg.naturalWidth > 0) {
        const box = relBox(mapImg, rootRect);
        ctx.drawImage(mapImg, box.x, box.y, box.w, box.h);
    }

    const leaderSvg = wrapperEl.querySelector('.electricity-prices-leader-lines');
    if (leaderSvg) {
    ...
    wrapperEl.querySelectorAll('.electricity-prices-city-label').forEach((el) => {
    ...
    const plotWrap = wrapperEl.querySelector('.electricity-prices-chart-overlay');
    ...
    const iconImgs = wrapperEl.querySelectorAll('.electricity-prices-city-icon');
