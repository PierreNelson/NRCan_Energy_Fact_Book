...
const Page126 = () => {
    ...
    const years = useMemo(() => PAGE126_DATA.map((d) => d.year), []);
    const counts = useMemo(() => PAGE126_DATA.map((d) => d.count), []);
    const depths = useMemo(() => PAGE126_DATA.map((d) => d.depth), []);

    const legendBars = getText('page126_legend_bars', lang);
    const legendLine = getText('page126_legend_line', lang);
    const yCount = getText('page126_yaxis_count', lang);
    const yDepth = getText('page126_yaxis_depth', lang);
    ...
    const exportTitleWithRange = `${stripHtml(getText('page126_title', lang))}${PAGE126_YEAR_RANGE_SUFFIX}`;
    ...
    const downloadTableAsCSV = () => {
        const headers = [
            ...
            getText('page126_table_col_count', lang),
            getText('page126_table_col_depth', lang)
        ];
        const rows = PAGE126_DATA.map((d) => [d.year, d.count, d.depth]);
    ...
    const downloadTableAsDocx = async () => {
        ...
        const cCol = getText('page126_table_col_count', lang);
        const dCol = getText('page126_table_col_depth', lang);
        ...
        const dataRows = PAGE126_DATA.map(
            (d) =>
                new TableRow({
                    children: [
                        new TableCell({
    ...
    return (
        <main
            ...
            role="main"
            aria-labelledby="page126-chart-title"
        ...
        >
            <style>{`
                ...
                .page126-legend-label {
                    font-size: 18px;
                    color: var(--gc-text);
                }
                .page126-table-wrapper {
                    display: block;
                    width: 100%;
                    margin-top: 20px;
                    margin-bottom: 0;
                }
                .page126-table-wrapper details > summary {
                    display: block;
                    width: 100%;
                    padding: 12px 15px;
                ...
                .page126-table-wrapper details > summary::-webkit-details-marker {
                    display: none;
                }
                .page126-table-wrapper details > summary:hover,
                .page126-chart-frame button[type="button"]:hover {
                    background-color: #404040 !important;
                }
                .page126-table-wrapper .table-responsive {
                    display: block;
                    width: 100%;
                    overflow-x: auto;
                ...
                .page126-table-wrapper .table-responsive table {
                    width: max-content;
                    min-width: 100%;
                }
                .page126-table-wrapper .table-responsive table th,
                .page126-table-wrapper .table-responsive table td {
                    white-space: nowrap;
                    padding: 8px 12px;
                    font-family: var(--font-body);
                    color: var(--gc-text);
                }
                .page126-download-buttons {
                    display: flex;
                    gap: 10px;
                    margin-top: 15px;
                    flex-wrap: wrap;
                }
                .page126-download-buttons button {
                    padding: 8px 16px;
                    border: 1px solid #404040;
                    border-radius: 4px;
                    background: #8C8C8C;
                ...
                .page126-download-buttons button:hover {
                    background: #404040 !important;
                }
                @media (max-width: 768px) {
                    .page126-chart-title { font-size: 26px; }
                    .page126-legend { flex-wrap: wrap; gap: 20px; }
                    .page126-legend-label { font-size: 16px; }
                }
                @media (max-width: 480px) {
                    .page126-legend-label { font-size: 14px; }
                }
            `}</style>

            <div className="page126-container">
                <div className="page126-chart-frame">
                    <h2 id="page126-chart-title" className="page126-chart-title">
                        {getText('page126_title', lang)}
                    </h2>

                    <div className="page126-chart-block">
                        <div role="region" aria-label={getText('page126_chart_aria', lang)} tabIndex="0">
                            {selectedPoints !== null && (
                            ...
                            <figure ref={chartRef} className="page126-chart" style={{ margin: 0 }}>
                        ...
                        <div className="page126-legend" aria-hidden="true">
                            <div className="page126-legend-inner">
                                <div className="page126-legend-item">
                                    <span className="page126-legend-swatch-bar" />
                                    <span className="page126-legend-label">{legendBars}</span>
                                </div>
                                <div className="page126-legend-item">
                                    <span className="page126-legend-line" />
                                    <span className="page126-legend-label">{legendLine}</span>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div className="page126-table-wrapper">
                        <details onToggle={(e) => setIsTableOpen(e.currentTarget.open)}>
                            ...
                            >
                                <table className="table table-bordered table-striped table-hover">
                                    <caption className="wb-inv">{getText('page126_table_caption', lang)}</caption>
                                    <thead>
                                        <tr>
                            ...
                            <div className="page126-download-buttons">
                ...
                <aside className="wb-fnote" role="note" style={{ marginTop: '12px', marginBottom: 0 }}>
                    <h2 id="fn-page126">{lang === 'en' ? 'Footnotes' : 'Notes de bas de page'}</h2>
                    <dl>
                        <dt>{lang === 'en' ? 'Footnote' : 'Note de bas de page'}</dt>
                        <dd style={{ marginBottom: 0 }}>
                            <p style={{ margin: 0 }}>{getText('page126_footnote', lang)}</p>
                        </dd>
                    </dl>
                </aside>
            </div>
