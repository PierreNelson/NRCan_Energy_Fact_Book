...
const UraniumRankingSection = ({
    ...
    const tableHeaders = [
        getText('page85_table_col_year', lang),
        getText(totalColKey, lang),
        ...TOP_RANKS.map((rank) => getText(`page85_table_col_rank${rank}`, lang)),
        getText('page85_table_col_canada', lang),
        getText('page85_table_col_canada_rank', lang),
    ];
    ...
    const renderCapsuleRow = (row, maxPct) => {
        ...
        const isCanada = row.isCanada;
        return (
            <div key={`${row.rank}-${row.name}`} className={`page85-capsule-row ${isCanada ? 'page85-capsule-row-canada' : ''}`}>
                <div className="page85-capsule-bar-wrap">
                    <div className="page85-capsule-bar" style={{ width: `${barWidthPct}%` }}>
                        <span className="page85-capsule-bar-text">{row.rank} {row.name}</span>
                    </div>
                </div>
                <div className="page85-capsule-connector" />
                <div className="page85-capsule-pill">
                    <span className={`page85-capsule-pill-text${isCanada ? ' page85-capsule-pill-text-canada' : ''}`}>{row.pctRounded}%</span>
                </div>
            </div>
        );
    };
    ...
    return (
        <section className="page85-chart-section" aria-labelledby={`${sectionId}-title`}>
            <h3 id={`${sectionId}-title`} className="page85-graphic-title">{graphicTitle}</h3>
            <div className="page85-capsule-graphic" role="img" aria-label={rankingAria}>
                {rankingRows.map((row) => renderCapsuleRow(row, maxPct))}
            </div>

            <div className="page85-download-buttons">
                <button type="button" onClick={downloadInfographicPng} style={downloadBtnStyle}>
                    {getText('page85_download_infographic_png', lang)}
                </button>
            </div>
            ...
            <details className="data-table-wrapper" onToggle={(e) => setIsTableOpen(e.currentTarget.open)}>
                <summary role="button" aria-expanded={isTableOpen}>
                    ...
                    {getText('page85_table_summary', lang)}
                </summary>
                <div ref={topScrollRef} className="page85-table-scrollbar" aria-hidden="true"><div /></div>
                <div ref={tableScrollRef} className="page85-table-responsive table-responsive" role="region" tabIndex={0}>
                ...
                <div ref={bottomScrollRef} className="page85-table-scrollbar" aria-hidden="true"><div /></div>
                <div className="page85-download-buttons">
                    <button type="button" onClick={downloadCsv} style={downloadBtnStyle}>
                        {getText('page85_download_csv', lang)}
                    </button>
                    <button type="button" onClick={downloadDocx} style={downloadBtnStyle}>
                        {getText('page85_download_docx', lang)}
                    </button>
                </div>
            </details>
        </section>
...
const Page85 = () => {
    ...
    return (
        <main
            ...
            role="main"
            aria-labelledby="page85-main-title"
...
.page85-container { width: 100%; padding: 15px 0 40px 0; box-sizing: border-box; }
.page85-title {
...
.page85-intro ul { margin: 0; padding-left: 1.25em; }
.page85-context-title {
    font-family: 'Lato', sans-serif;
    font-size: 22px;
    font-weight: bold;
    color: var(--gc-text);
...
.page85-chart-section { margin-bottom: 36px; }
.page85-graphic-title {
    font-family: 'Lato', sans-serif;
    font-size: 20px;
    font-weight: bold;
    color: var(--gc-text);
...
.page85-capsule-bar-wrap { flex: 0 0 28%; min-width: 0; display: flex; align-items: center; }
.page85-capsule-bar {
    min-height: 44px;
    min-width: 82px;
    border-radius: 999px;
...
.page85-capsule-row-canada .page85-capsule-bar { background-color: ${CANADA_CAPSULE}; color: #fff; font-weight: bold; }
.page85-capsule-bar-text { word-wrap: break-word; overflow-wrap: break-word; word-break: break-word; white-space: normal; line-height: 1.3; }
.page85-capsule-connector { flex: 1 1 0; min-width: 10px; height: 2px; background-color: #b0b0b0; }
.page85-capsule-row-canada .page85-capsule-connector { background-color: ${CANADA_CAPSULE}; }
.page85-capsule-pill {
    flex: 0 0 64px;
    height: 36px;
    min-width: 64px;
...
.page85-capsule-pill-text { font-size: 15px; }
.page85-capsule-row-canada .page85-capsule-pill { border-width: 2px; border-color: ${CANADA_CAPSULE}; font-weight: bold; }
.page85-capsule-pill-text-canada { font-size: 16px; font-weight: bold; }
.page85-table-responsive {
    display: block;
    width: 100%;
    overflow-x: auto;
...
.page85-table-responsive::-webkit-scrollbar { display: none; }
.page85-table-responsive table { width: max-content !important; min-width: 100%; }
.page85-download-buttons { display: flex; gap: 10px; flex-wrap: wrap; margin-top: 15px; }
.page85-loading, .page85-error { font-family: Arial, sans-serif; font-size: 16px; color: var(--gc-text); }
.page85-chart-section .data-table-wrapper { padding-top: 5px; margin-top: 20px; width: 100%; box-sizing: border-box; }
...
.page85-table-scrollbar { width: 100%; overflow-x: auto; overflow-y: hidden; margin: 0; }
.page85-table-scrollbar > div { height: 20px; }
@media (max-width: 480px) {
    .page85-title { font-size: 28px; }
}
            `}</style>

            <div className="page85-container">
                {loading && <p className="page85-loading">{lang === 'en' ? 'Loading data…' : 'Chargement des données…'}</p>}
                {!loading && error && <p className="page85-error" role="alert">{error}</p>}
                {!loading && !error && !hasData && (
                    <p className="page85-error">{lang === 'en' ? 'No data available.' : 'Aucune donnée disponible.'}</p>
                )}

                {hasData && (
                    <>
                        <header>
                            <h1 id="page85-main-title" className="page85-title">{getText('page85_title', lang)}</h1>
                        </header>

                        <div className="page85-intro">
                            <ul>
                                <li dangerouslySetInnerHTML={{ __html: getText('page85_bullet1', lang) }} />
                            </ul>
                        </div>

                        <h2 className="page85-context-title">{getText('page85_context_title', lang)}</h2>

                        <UraniumRankingSection
                            sectionId="page85-exports"
                            series={result.exports}
                            chartTitleKey="page85_exports_chart_title"
                            tableCaptionKey="page85_exports_table_caption"
                            totalColKey="page85_table_col_total_exports"
                            csvSlugKey="page85_exports_csv_slug"
                            docxSlugKey="page85_exports_docx_slug"
                            pngSlugKey="page85_exports_png_slug"
                            lang={lang}
                            locale={locale}
                        ...
                        <UraniumRankingSection
                            sectionId="page85-production"
                            series={result.production}
                            chartTitleKey="page85_production_chart_title"
                            tableCaptionKey="page85_production_table_caption"
                            totalColKey="page85_table_col_total_production"
                            csvSlugKey="page85_production_csv_slug"
                            docxSlugKey="page85_production_docx_slug"
                            pngSlugKey="page85_production_png_slug"
                            lang={lang}
                            locale={locale}
                            windowWidth={windowWidth}
                        />
                        ...
                        <UraniumRankingSection
                            sectionId="page85-resources"
                            series={result.resources}
                            chartTitleKey="page85_resources_chart_title"
                            tableCaptionKey="page85_resources_table_caption"
                            totalColKey="page85_table_col_total_resources"
                            csvSlugKey="page85_resources_csv_slug"
                            docxSlugKey="page85_resources_docx_slug"
                            pngSlugKey="page85_resources_png_slug"
                            lang={lang}
                            locale={locale}
                            windowWidth={windowWidth}
                        />
