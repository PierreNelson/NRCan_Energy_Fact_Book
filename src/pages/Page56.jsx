...
const PAGE56_VERTICAL_TICK_ZOOM = 2.5;
const PAGE56_OPERATOR_OVERLAP_ZOOM = 4.0;
...
function isBrowserZoomInVerticalTickRange() {
    ...
    return LIKELY_OS_DPR_BASES.some((b) => {
        const z = dpr / b;
        return z >= PAGE56_VERTICAL_TICK_ZOOM && z <= 6.0;
    });
};
...
const Page56 = () => {
    ...
    const categoryLabels = useMemo(
        () => BAR_KEYS.map((key) => getText(`page56_cat_${key}`, lang)),
        [lang],
    );

    const values = useMemo(() => BAR_KEYS.map((key) => PAGE56_DATA[key]), []);
    ...
    const chartTitle = getText('page56_chart_title', lang);
    const exportChartTitle = stripHtml(chartTitle);
    const chartDownloadSlug = getText('page56_download_title', lang).replace(/\s+/g, '_');
    const yAxisTitle = getText('page56_yaxis', lang);
    ...
    const pinVertical =
        viewportZoom.pinScale > 1.02 && viewportZoom.pinScale >= PAGE56_VERTICAL_TICK_ZOOM;
    const useVerticalXLabels =
        pageZoomForLayout >= PAGE56_VERTICAL_TICK_ZOOM ||
        pinVertical ||
    ...
    const barGap = useMemo(() => {
        if (pageZoomForLayout >= PAGE56_OPERATOR_OVERLAP_ZOOM) return 0.52;
        if (pageZoomForLayout >= 3) return 0.38;
        if (pageZoomForLayout >= PAGE56_VERTICAL_TICK_ZOOM) return 0.30;
        return 0.22;
    }, [pageZoomForLayout]);
    ...
    const operatorFontSize = useMemo(() => {
        ...
        if (pageZoomForLayout >= PAGE56_OPERATOR_OVERLAP_ZOOM) return Math.max(12, Math.round(base * 0.62));
    ...
    const tableHeaders = [
        getText('page56_table_col_factor', lang),
        `${yAxisTitle} (PJ)`,
    ];
    ...
    const scrollToFootnote = (e) => {
        e.preventDefault();
        document.getElementById('fn-asterisk-page56')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    };

    const scrollToRef = (e) => {
        e.preventDefault();
        document.getElementById('fn-asterisk-rf-page56')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    };
    ...
    return (
        <main
            id="main-content"
            tabIndex="-1"
            className="page-content page-56 page56-energy-factors"
            role="main"
            aria-labelledby="page56-chart-title"
            style={{ backgroundColor: '#ffffff' }}
        >
            <style>{`
.page-56.page56-energy-factors {
...
.page56-inner { width: 100%; padding: 15px 0 40px 0; box-sizing: border-box; }
.page56-bullets {
...
.page56-bullets li { margin-bottom: 0.65rem; }
.page56-chart-frame {
...
.page56-chart > div { width: 100%; height: 100%; }
.page56-chart .xaxislayer-above path.domain,
.page56-chart .xaxislayer-below path.domain {
    stroke: transparent !important;
    stroke-width: 0 !important;
}
.page56-table-wrapper { display: block; width: 100%; margin-top: 20px; }
.page56-table-wrapper details > summary {
    display: block;
    width: 100%;
    padding: 12px 15px;
...
.page56-table-wrapper details > summary::-webkit-details-marker { display: none; }
.page56-table-wrapper details > summary:hover,
.page56-chart-frame button[type="button"]:hover { background-color: #404040 !important; }
.page56-table-scrollbar { width: 100%; overflow-x: auto; overflow-y: hidden; margin: 0; }
.page56-table-scrollbar > div { height: 20px; }
.page56-table-responsive {
    display: block;
    width: 100%;
    overflow-x: auto;
...
.page56-table-responsive::-webkit-scrollbar { display: none; }
.page56-table-responsive table { width: max-content; min-width: 100%; border-collapse: collapse; }
.page56-table-responsive th,
.page56-table-responsive td {
    padding: 8px 12px;
    border: 1px solid #ddd;
    font-family: Arial, sans-serif;
    color: var(--gc-text);
...
.page56-download-buttons { display: flex; gap: 10px; margin-top: 15px; flex-wrap: wrap; }
.page56-download-buttons button {
    padding: 8px 16px;
    border: 1px solid #404040;
    border-radius: 4px;
    background: #8C8C8C;
...
@media (max-width: 768px) {
    .page56-bullets { font-size: 18px; }
    .page56-chart-title { font-size: 26px; }
    .page56-footnotes { font-size: 0.9rem; }
    .page56-footnotes h2 { font-size: 1.2rem; margin-bottom: 0.75rem; }
}
            `}</style>

            <div className="page56-inner">
                <ul className="page56-bullets" role="list">
                    {bulletKeys.map((key) => (
                        <li key={key} role="listitem">
                            <strong>{getText(`page56_bullet_${key}_bold`, lang)}</strong>
                            {getText(`page56_bullet_${key}_text`, lang)}
                        </li>
                    ))}
                </ul>

                <div className="page56-chart-frame">
                    <h2 id="page56-chart-title" className="page56-chart-title">
                        {chartTitle}
                        <sup id="fn-asterisk-rf-page56">
                            <a className="fn-lnk" href="#fn-asterisk-page56" onClick={scrollToFootnote}>
                    ...
                    <figure ref={chartRef} style={{ margin: 0 }}>
                        <div role="region" aria-label={getText('page56_chart_aria', lang)} tabIndex="0">
                            <Plot
                                key={`page56-${selectedBars ? selectedBars.join('-') : 'all'}-${useVerticalXLabels ? 'v' : 'h'}-${plotHeight}-${barGap.toFixed(2)}-${operatorFontSize}`}
                                data={[
                                    {
                                        x: BAR_KEYS,
                                        y: values,
                    ...
                    <div className="page56-table-wrapper">
                        <details onToggle={(e) => setIsTableOpen(e.currentTarget.open)}>
                            ...
                            <div ref={topScrollRef} className="page56-table-scrollbar" aria-hidden="true">
                                <div />
                            </div>
                            <div ref={tableScrollRef} className="page56-table-responsive" role="region" tabIndex="0">
                                <table className="table table-striped table-hover">
                                    <caption className="wb-inv">{getText('page56_table_caption', lang)}</caption>
                                    <thead>
                                        <tr>
                                ...
                                <div className="page56-download-buttons">
                            ...
                            <div ref={bottomScrollRef} className="page56-table-scrollbar" aria-hidden="true">
                                <div />
                            </div>
                        </details>
                    </div>
                ...
                <aside className="wb-fnote page56-footnotes" role="note">
                    <h2>{lang === 'en' ? 'Footnotes' : 'Notes de bas de page'}</h2>
                    <dl>
                        <dt id="fn-asterisk-page56" className="wb-inv">
                            {lang === 'en' ? 'Footnote 1' : 'Note de bas de page 1'}
                        </dt>
                        <dd>
                            <a
                                href="#fn-asterisk-rf-page56"
                                onClick={scrollToRef}
                                className="fn-num"
