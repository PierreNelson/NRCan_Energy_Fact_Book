...
const ElectricityGhgSpotlightPlot = memo(function ElectricityGhgSpotlightPlot({
    plotData, plotLayout, chartConfig, plotHeight, onPlotReady,
}) {
    return (
        <Plot
            key={`electricity-ghg-spotlight-${plotHeight}`}
            data={plotData}
            layout={plotLayout}
            config={chartConfig}
...
const ElectricityGhgSpotlight = () => {
    ...
    const tableUnitSuffix = getText('ghg_electricity_spotlight_table_unit', lang);
    ...
    const intro1Bold = result?.totalPctChange != null
        ...
        : getText('ghg_electricity_spotlight_intro1_bold', lang);
    ...
    const intro2Bold1 = result?.coalGenSharePct != null
        ...
        : getText('ghg_electricity_spotlight_intro2_bold1', lang);
    ...
    const intro2Bold2 = result?.coalGhgSharePct != null
        ...
        : getText('ghg_electricity_spotlight_intro2_bold2', lang);

    const textVars = { startYear: startYear ?? '', endYear: endYear ?? '', baseYear: baseYear ?? '', referenceYear: referenceYear ?? '' };
    const intro1Part2 = substitute(getText('ghg_electricity_spotlight_intro1_part2', lang), textVars);
    const intro2Part3 = substitute(getText('ghg_electricity_spotlight_intro2_part3', lang), textVars);

    const coalLabel = getText('ghg_electricity_spotlight_legend_coal', lang);
    const naturalGasLabel = getText('ghg_electricity_spotlight_legend_naturalGas', lang);
    const otherLabel = getText('ghg_electricity_spotlight_legend_other', lang);
    const chartTitle = substitute(getText('ghg_electricity_spotlight_chart_title', lang), textVars);
    const yAxisTitle = getText('ghg_electricity_spotlight_yaxis', lang);
    const fileTitle = `${getText('ghg_electricity_spotlight_download_title', lang)}_${startYear ?? ''}-${endYear ?? ''}`;
    const tableCaption = substitute(getText('ghg_electricity_spotlight_table_caption', lang), textVars);

    const tableHeaders = [
        getText('ghg_electricity_spotlight_table_col_year', lang),
        `${coalLabel} ${tableUnitSuffix}`,
        `${naturalGasLabel} ${tableUnitSuffix}`,
        `${otherLabel} ${tableUnitSuffix}`,
        `${getText('ghg_electricity_spotlight_table_col_total', lang)} ${tableUnitSuffix}`,
    ];
    ...
    const bindPlotHandlers = useCallback((graphDiv) => {
        ...
        if (graphDiv._electricityGhgSpotlightClick) {
            graphDiv.removeListener('plotly_click', graphDiv._electricityGhgSpotlightClick);
        }
        const clickHandler = (event) => clickHandlerRef.current?.(event);
        graphDiv._electricityGhgSpotlightClick = clickHandler;
        ...
        try {
            ...
            img.onload = () => {
                ...
                const legendItems = TRACE_KEYS.map((key, index) => ({
                    color: COLORS[key],
                    label: getText(`ghg_electricity_spotlight_legend_${key}`, lang),
                    index,
                }));
    ...
    if (loading) {
        return <p className="electricity-ghg-spotlight-loading">{lang === 'en' ? 'Loading data…' : 'Chargement des données…'}</p>;
    }
    if (error) {
        return <p className="electricity-ghg-spotlight-error" role="alert">{error}</p>;
    }
    if (!tableRows.length) {
        return <p className="electricity-ghg-spotlight-error">{lang === 'en' ? 'No data available.' : 'Aucune donnée disponible.'}</p>;
    }
    ...
    return (
        <main
            ...
            role="main"
            aria-labelledby="electricity-ghg-spotlight-title"
...
.electricity-ghg-spotlight-inner { width: 100%; padding: 15px 0 40px 0; box-sizing: border-box; }
.electricity-ghg-spotlight-title {
...
.electricity-ghg-spotlight-title::after {
    content: '';
    position: absolute;
    left: 0;
    bottom: 0.2em;
...
.electricity-ghg-spotlight-intro {
...
.electricity-ghg-spotlight-chart-frame {
...
.electricity-ghg-spotlight-chart-title {
    font-family: 'Lato', sans-serif;
    font-size: 29px;
    font-weight: bold;
    color: var(--gc-text);
...
.electricity-ghg-spotlight-chart { width: 100%; min-width: 0; height: ${plotHeight}px; position: relative; }
.electricity-ghg-spotlight-chart > div { width: 100%; height: 100%; }
.electricity-ghg-spotlight-legend {
    display: flex;
...
.electricity-ghg-spotlight-legend-item { display: inline-flex; align-items: center; gap: 8px; white-space: nowrap; }
.electricity-ghg-spotlight-legend-swatch { width: 22px; height: 14px; display: inline-block; border: 1px solid rgba(0, 0, 0, 0.12); }
.electricity-ghg-spotlight-download-buttons { display: flex; gap: 10px; flex-wrap: wrap; margin-top: 15px; }
.electricity-ghg-spotlight-download-buttons button:hover,
...
.electricity-ghg-spotlight-table-scrollbar { width: 100%; overflow-x: auto; overflow-y: hidden; margin: 0; }
.electricity-ghg-spotlight-table-scrollbar > div { height: 20px; }
@media (max-width: 768px) {
    .electricity-ghg-spotlight-title { font-size: 37px; }
    .electricity-ghg-spotlight-intro { font-size: 18px; }
    .electricity-ghg-spotlight-chart-title { font-size: 26px; }
}
.electricity-ghg-spotlight-loading, .electricity-ghg-spotlight-error {
    font-family: Arial, sans-serif;
    font-size: 16px;
    color: var(--gc-text);
    padding: 24px 0;
            ...
            <div className="electricity-ghg-spotlight-inner">
                <h1 id="electricity-ghg-spotlight-title" className="electricity-ghg-spotlight-title">{getText('ghg_electricity_spotlight_title', lang)}</h1>

                <p className="electricity-ghg-spotlight-intro">
                    {getText('ghg_electricity_spotlight_intro1_part1', lang)}
                    <strong>{intro1Bold}</strong>
                    {intro1Part2}
                </p>
                <p className="electricity-ghg-spotlight-intro">
                    {getText('ghg_electricity_spotlight_intro2_part1', lang)}
                    <strong>{intro2Bold1}</strong>
                    {getText('ghg_electricity_spotlight_intro2_part2', lang)}
                    <strong>{intro2Bold2}</strong>
                    {intro2Part3}
                    {lang === 'fr' && (
                        <>
                            <strong>{getText('ghg_electricity_spotlight_intro2_bold3', lang)}</strong>
                            {substitute(getText('ghg_electricity_spotlight_intro2_part4', lang), textVars)}
                        </>
                    )}
                </p>

                <div className="electricity-ghg-spotlight-chart-frame">
                    <h2 id="electricity-ghg-spotlight-chart-title" className="electricity-ghg-spotlight-chart-title">{chartTitle}</h2>
                    ...
                    <figure ref={chartRef} className="electricity-ghg-spotlight-chart" role="region" aria-label={chartTitle} tabIndex={0} style={{ margin: 0 }}>
                        <ElectricityGhgSpotlightPlot
                            plotData={plotData}
                            plotLayout={{
                                barmode: 'stack',
                                showlegend: false,
                            ...
                            chartConfig={{
                                ...
                                modeBarButtonsToAdd: [{
                                    name: getText('ghg_electricity_spotlight_download_png', lang),
                                    icon: {
                                        width: 24,
                                        height: 24,
                    ...
                    <div className="electricity-ghg-spotlight-legend" aria-hidden="true">
                        {TRACE_KEYS.map((key) => (
                            <span key={key} className="electricity-ghg-spotlight-legend-item">
                                <span className="electricity-ghg-spotlight-legend-swatch" style={{ backgroundColor: COLORS[key] }} />
                                {getText(`ghg_electricity_spotlight_legend_${key}`, lang)}
                            </span>
                        ))}
                    </div>
                    ...
                    <details className="data-table-wrapper" onToggle={(e) => setIsTableOpen(e.currentTarget.open)}>
                        ...
                        <div ref={topScrollRef} className="electricity-ghg-spotlight-table-scrollbar" aria-hidden="true"><div /></div>
                        <div
                            ref={tableScrollRef}
                        ...
                        <div ref={bottomScrollRef} className="electricity-ghg-spotlight-table-scrollbar" aria-hidden="true"><div /></div>
                        <div className="electricity-ghg-spotlight-download-buttons">
                            <button type="button" onClick={downloadCsv} style={downloadBtnStyle}>
                                {getText('ghg_electricity_spotlight_download_csv', lang)}
                            </button>
                            <button type="button" onClick={downloadDocx} style={downloadBtnStyle}>
                                {getText('ghg_electricity_spotlight_download_docx', lang)}
                            </button>
                        </div>
                    </details>
                </div>
...
export default ElectricityGhgSpotlight;
