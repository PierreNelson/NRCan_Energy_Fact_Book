...
const PAGE49_KEYS = ['Ele', 'NG', 'mogas', 'Oil', 'OOP', 'BM', 'OT'];
const PAGE49_LABEL_KEYS = {
    Ele: 'page49_label_electricity',
    NG: 'page49_label_natural_gas',
    mogas: 'page49_label_motor_gasoline',
    Oil: 'page49_label_oil',
    OOP: 'page49_label_other_oil',
    BM: 'page49_label_biomass',
...
const Page49 = () => {
    ...
    const donutData = useMemo(() => {
        ...
        const labels = categories.map((c) => getText(PAGE49_LABEL_KEYS[c.key], lang));
        const hoverTexts = categories.map((c) => {
            const label = getText(PAGE49_LABEL_KEYS[c.key], lang);
    ...
    const plotData = useMemo(() => {
        ...
        const getColors = () => (effectiveSelectedSlices === null ? PAGE49_COLORS : PAGE49_COLORS.map((c, i) => (effectiveSelectedSlices.includes(i) ? c : hexToRgba(c, 0.3))));
    ...
    const layout = useMemo(() => ({
        ...
        annotations: donutData.total != null ? [{
            text: `${getText('page49_total', lang)}<br><b>${(donutData.total || 0).toLocaleString()}</b><br>PJ`,
            showarrow: false,
            x: 0.5,
            y: 0.5,
            xref: 'paper',
    ...
    const scrollToFootnote = (e) => {
        e?.preventDefault();
        document.getElementById('fn-asterisk-page49')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    };

    const scrollToRef = (e) => {
        e?.preventDefault();
        document.getElementById('fn-asterisk-rf-page49')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    };
...
.page49-chart-frame button:hover { background-color: #404040 !important; }
            ...
            <ul className="page49-bullets">
                ...
                {hasData && TE != null && (
                    <li>
                        {(getText('page49_bullet2_was', lang) || '').replace(/\{\{year\}\}/g, String(selectedYear))}
                        <strong>{TE.toLocaleString()}</strong>
                        {getText('page49_bullet2_pj', lang)}
                    </li>
                )}
                {hasData && TEX != null && EleX != null && NGX != null && (
                    <li>
                        {getText('page49_bullet3_prefix', lang)}
                        <strong>{TEX >= 0 ? getText('page49_bullet3_increased_word', lang) : getText('page49_bullet3_decreased_word', lang)}</strong>
                        {' '}
                        <strong>{Math.abs(TEX)}%</strong>
                        {getText('page49_bullet3_mid1', lang)}{selectedYear}.{getText('page49_bullet3_mid2', lang)}
                        <strong>{NGX}%</strong>
                        {getText('page49_bullet3_mid3', lang)}
                        <strong>{EleX}%</strong>
                        {getText('page49_bullet3_suffix', lang)}
                    </li>
                )}
            </ul>
            ...
            {hasData && (
                <>
                    <div className="page49-charts-anchor" style={{ width: '100%', boxSizing: 'border-box' }}>
                        <div className="page49-chart-wrap" ref={chartRef}>
                            <div className="page49-chart-frame" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                                <p className="page49-chart-title" style={{ textAlign: 'center', width: '100%', boxSizing: 'border-box', margin: '0 0 1rem 0' }}>
                                    {chartTitle}
                                    <span id="fn-asterisk-rf-page49" style={{ verticalAlign: 'super', fontSize: '0.75em' }}>
                                        <a className="fn-lnk" href="#fn-asterisk-page49" onClick={scrollToFootnote}>
                    ...
                    <aside className="wb-fnote page49-footnote" role="note">
                        <h2 id="fn">{lang === 'en' ? 'Footnotes' : 'Notes de bas de page'}</h2>
                        <dl>
                            <dt className="wb-inv">{lang === 'en' ? 'Footnotes' : 'Notes de bas de page'}</dt>
                            <dd id="fn-asterisk-page49">
                                <a href="#fn-asterisk-rf-page49" onClick={scrollToRef} className="fn-num" title={lang === 'en' ? 'Return to footnote * referrer' : 'Retour à la référence de la note de bas de page *'}>
                                    <span className="wb-inv">{lang === 'en' ? 'Return to footnote ' : 'Retour à la note de bas de page '}</span>*
                                </a>
                                {' '}{getText('page49_footnote_other', lang)}
                            </dd>
                            <dd>
                                {getText('page49_footnote_rounding', lang)}
                            </dd>
                        </dl>
                    </aside>
                </>
