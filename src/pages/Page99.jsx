...
const getPageZoomScale = () => {
    ...
    const dpr = window.devicePixelRatio || 1;
    return Math.max(viewportScale, dpr);
};

const Page99Plot = memo(function Page99Plot({
    plotData, plotLayout, chartConfig, plotHeight, plotKey,
...
const Page99 = () => {
    ...
    const [browserZoom, setBrowserZoom] = useState(() => computePageZoomScale(null).zoomScale);
    ...
    const pieTextVars = { year: selectedPieYear };
    const barTextVars = { startYear: BAR_START_YEAR, endYear: BAR_END_YEAR };

    const pieChartTitle = substitute(getText('page99_pie_chart_title', lang), pieTextVars);
    const barChartTitle = getText('page99_bar_chart_title', lang);
    const pieFileTitle = substitute(getText('page99_pie_download_title', lang), pieTextVars);
    const barFileTitle = substitute(getText('page99_bar_download_title', lang), barTextVars);
    const pieTableCaption = substitute(getText('page99_pie_table_caption', lang), pieTextVars);
    const barTableCaption = substitute(getText('page99_bar_table_caption', lang), barTextVars);
    ...
    const pieLabels = pieSlices.map((slice) => getText(`page99_legend_${slice.key}`, lang));
    ...
    const outsideLabelTemplate = pieSlices.map((slice) => {
        ...
        if (slice.key === 'level3') {
            return `${getText('page99_legend_level3_line1', lang)}<br>${getText('page99_legend_level3_line2', lang)}${labelPctGap}${pctLabelTemplate}`;
        }
    ...
    const pieTrace = pieSlices.length ? {
        ...
        hovertext: pieSlices.map((slice) => {
            const label = getText(`page99_legend_${slice.key}`, lang);
    ...
    const pieCenterAnnotation = pieRow?.total != null ? {
        text: `<span style="font-size:${pieCenterValueSize}px"><b>${getText('page99_pie_center_value', lang)}</b></span><br><span style="font-size:${pieCenterLabelSize}px">${getText('page99_pie_center_label_line1', lang)}</span><br><span style="font-size:${pieCenterLabelSize}px">${getText('page99_pie_center_label_line2', lang)}</span>`,
        showarrow: false,
        x: 0.5,
        y: 0.5,
        xref: 'paper',
    ...
    const barLayout = {
        ...
        yaxis: {
            title: { text: getText('page99_bar_yaxis', lang), font: { size: windowWidth <= 768 ? 15 : 16, family: 'Arial, sans-serif', color: '#58585a' }, standoff: 12 },
            range: [0, 14],
            dtick: 2,
            tickfont: { size: windowWidth <= 480 ? 12 : 14, family: 'Arial, sans-serif' },
            fixedrange: true,
    ...
    const pieTableHeaders = [
        getText('page99_pie_table_col_year', lang),
        getText('page99_pie_table_col_type', lang),
        getText('page99_pie_table_col_share', lang),
    ];

    const barTableHeaders = [
        getText('page99_bar_table_col_year', lang),
        getText('page99_bar_table_col_stations', lang),
        getText('page99_bar_table_col_growth', lang),
    ];

    const pieTableRows = pieSlices.map((slice) => ({
        year: selectedPieYear,
        type: getText(`page99_legend_${slice.key}`, lang),
        share: slice.pct,
    }));
    ...
    const bindPiePlotHandlers = useCallback((graphDiv) => {
        ...
        if (graphDiv._page99PieClick) {
            graphDiv.removeListener('plotly_click', graphDiv._page99PieClick);
        }
        const clickHandler = (event) => pieClickHandlerRef.current?.(event);
        graphDiv._page99PieClick = clickHandler;
    ...
    const bindBarPlotHandlers = useCallback((graphDiv) => {
        ...
        if (graphDiv._page99BarClick) {
            graphDiv.removeListener('plotly_click', graphDiv._page99BarClick);
        }
        const clickHandler = (event) => barClickHandlerRef.current?.(event);
        graphDiv._page99BarClick = clickHandler;
        graphDiv.on('plotly_click', clickHandler);
    }, []);
    ...
    const piePlotKey = `page99-pie-${selectedPieYear}-${useStackedLayout ? 'stacked' : 'side'}-${zoomLegendMode ? 'zoom' : 'full'}-${pieColumnWidth}`;
    const barPlotKey = `page99-bar-${useStackedLayout ? 'stacked' : 'side'}-${barPlotHeight}`;
    ...
    const pieChartConfig = useMemo(() => ({
        ...
        modeBarButtonsToAdd: [{
            name: getText('page99_download_png', lang),
    ...
    const footnoteLink = (referrerId) => (
        <span id={referrerId} style={{ verticalAlign: 'super', fontSize: '0.75em' }}>
            <a className="fn-lnk" href="#fn-asterisk-page99" onClick={scrollToElement('fn-asterisk-page99')}>
    ...
    const getPieLegendLabel = (sliceKey) => getText(`page99_legend_${sliceKey}`, lang);

    const renderPieLegendItem = (slice) => (
        <span key={slice.key} className="page99-custom-legend-item">
            <span className="page99-custom-legend-swatch" style={{ backgroundColor: SLICE_COLORS[slice.key] }} aria-hidden="true" />
    ...
    return (
        <main
            ...
            role="main"
            aria-labelledby="page99-pie-chart-title"
...
.page99-inner { width: 100%; padding: 15px 0 40px 0; box-sizing: border-box; }
.page99-content-row {
    display: flex;
    flex-direction: row;
    width: 100%;
...
.page99-chart-title .fn-lnk { color: var(--gc-text); }
.page99-chart-scroll { width: 100%; overflow: visible; display: flex; justify-content: center; align-items: center; }
.page99-chart {
    width: 100%;
    min-width: 0;
    min-height: 320px;
...
.page99-chart > div { width: 100%; height: 100%; overflow: visible; }
.page99-chart .js-plotly-plot,
.page99-chart .plot-container,
.page99-chart .svg-container { overflow: visible !important; }
.page99-chart .plotly .slice path.textline {
...
.page99-level1-label .page99-level1-pct {
    font-size: ${piePctFontSize}px;
    font-weight: bold;
    line-height: 1.1;
}
.page99-clear-selection {
    padding: 6px 12px;
...
.page99-custom-legend-item { display: inline-flex; align-items: center; gap: 6px; }
.page99-custom-legend-swatch {
    width: 14px;
    height: 14px;
...
.page99-download-buttons { display: flex; gap: 10px; flex-wrap: wrap; margin-top: 15px; }
.page99-download-buttons button:hover,
.page-99 .data-table-wrapper summary:hover,
.page-99 .data-table-wrapper button:hover,
.page99-clear-selection:hover { background-color: #404040 !important; }
.page-99 .data-table-wrapper { padding-top: 5px; margin-top: 20px; width: 100%; box-sizing: border-box; }
.page-99 .data-table-wrapper.page99-panel-footer { margin-top: auto; padding-top: 20px; }
...
.page99-table-scrollbar { width: 100%; overflow-x: auto; overflow-y: hidden; margin: 0; }
.page99-table-scrollbar > div { height: 20px; }
.page99-footnotes {
...
.page99-footnotes dd { display: flex; align-items: flex-start; gap: 0.75rem; margin-left: 0; margin-bottom: 1rem; }
.page99-footnotes dd p { margin: 0; }
.layout-stacked.page99-content-row { flex-direction: column !important; }
.layout-stacked .page99-pie-column,
.layout-stacked .page99-bar-column {
    width: 100% !important;
    min-width: 0 !important;
    padding: 0 !important;
}
.layout-stacked .page99-pie-column-side { background-color: transparent; padding: 0; }
.layout-stacked .page99-bar-column { margin-top: 28px; }

@media (max-width: 768px) {
    .page99-chart-title { font-size: 22px; }
    .page99-paragraph { font-size: 18px; }
    .page99-level1-label { left: calc(50% + 24px); top: 40px; }
    .page99-level1-leader::before { height: 42px; }
}
@media (max-width: 480px) {
    .page99-level1-label { left: calc(50% + 4px); top: 44px; }
    .page99-level1-leader { width: 3px; }
    .page99-level1-leader::before { height: 36px; }
    .page99-level1-name { font-size: 12px; }
}
            `}</style>

            <div className="page99-inner">
                <div ref={contentRowRef} className={`page99-content-row ${useStackedLayout ? 'layout-stacked' : ''}`}>
                    <div className={`page99-pie-column ${useStackedLayout ? '' : 'page99-pie-column-side'}`}>
                        <div className={useStackedLayout ? 'page99-chart-frame' : 'page99-pie-panel'}>
                            <h2 id="page99-pie-chart-title" className="page99-chart-title">
                                {substitute(getText('page99_pie_chart_title_plain', lang), pieTextVars)}
                                {footnoteLink('fn-asterisk-rf-page99-pie')}
                            </h2>

                            {effectivePieSlices?.length > 0 && (
                                <button type="button" className="page99-clear-selection" onClick={() => setSelectedPieSlices(null)}>
                                    {lang === 'en' ? 'Clear selection' : 'Effacer la sélection'}
                                </button>
                            )}

                            <div className="page99-chart-block page99-chart-scroll">
                                <figure
                                    ref={pieChartRef}
                                    className="page99-chart"
                                    role="region"
                                ...
                                >
                                    ...
                                    {!zoomLegendMode && level1Slice && (
                                        <span className="page99-level1-label" aria-hidden="true">
                                            <span className="page99-level1-row">
                                                <span className="page99-level1-leader" />
                                                <span className="page99-level1-name">{getText('page99_legend_level1', lang)}</span>
                                            </span>
                                            <span className="page99-level1-pct">{formatPct(level1Slice.pct)}</span>
                                        </span>
                                    )}
                                </figure>
                            </div>
                            ...
                            {zoomLegendMode && (
                                <div className="page99-custom-legend" aria-label={lang === 'en' ? 'Chart legend' : 'Légende du graphique'}>
                                    {pieSlices.map(renderPieLegendItem)}
                                </div>
                            )}

                            <details className="data-table-wrapper page99-panel-footer" onToggle={(e) => setIsPieTableOpen(e.currentTarget.open)}>
                                ...
                                <div ref={pieTopScrollRef} className="page99-table-scrollbar" aria-hidden="true"><div /></div>
                                <div
                                ...
                                <div ref={pieBottomScrollRef} className="page99-table-scrollbar" aria-hidden="true"><div /></div>
                                <div className="page99-download-buttons">
                                    <button type="button" onClick={downloadPieCsv} style={downloadBtnStyle}>
                                        {getText('page99_download_csv', lang)}
                                    </button>
                                    <button type="button" onClick={downloadPieDocx} style={downloadBtnStyle}>
                                        {getText('page99_download_docx', lang)}
                                    </button>
                                </div>
                            </details>
                        </div>
                    ...
                    <div className="page99-bar-column">
                        <div className="page99-chart-frame">
                            <h2 id="page99-bar-chart-title" className="page99-chart-title">
                                {getText('page99_bar_chart_title_plain', lang)}
                                {footnoteLink('fn-asterisk-rf-page99-bar')}
                            </h2>

                            {selectedBarPoints !== null && (
                                <button type="button" className="page99-clear-selection" onClick={() => setSelectedBarPoints(null)}>
                                    {lang === 'en' ? 'Clear selection' : 'Effacer la sélection'}
                                </button>
                            )}

                            <div className="page99-chart-block">
                                <figure
                                    ref={barChartRef}
                                    className="page99-chart"
                                    role="region"
                            ...
                            <details className="data-table-wrapper page99-panel-footer" onToggle={(e) => setIsBarTableOpen(e.currentTarget.open)}>
                                ...
                                <div ref={barTopScrollRef} className="page99-table-scrollbar" aria-hidden="true"><div /></div>
                                <div
                                ...
                                <div ref={barBottomScrollRef} className="page99-table-scrollbar" aria-hidden="true"><div /></div>
                                <div className="page99-download-buttons">
                ...
                <p className="page99-paragraph">
                    {getText('page99_paragraph_part1', lang)}
                    <strong>{getText('page99_paragraph_bold', lang)}</strong>
                    {getText('page99_paragraph_part2', lang)}
                </p>

                <aside className="wb-fnote page99-footnotes" role="note">
                    <h2>{lang === 'en' ? 'Footnotes' : 'Notes de bas de page'}</h2>
                    <dl>
                        <dt className="wb-inv">{lang === 'en' ? 'Footnote' : 'Note de bas de page'}</dt>
                        <dd id="fn-asterisk-page99">
                            <a
                                href="#fn-asterisk-rf-page99-pie"
                                onClick={scrollToElement('fn-asterisk-rf-page99-pie')}
                                className="fn-num"
                            ...
                            {' '}
                            <p>{getText('page99_footnote', lang)}</p>
                        </dd>
                    </dl>
                </aside>
            </div>
