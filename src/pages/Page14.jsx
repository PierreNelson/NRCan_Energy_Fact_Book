 ...
 */
function page14MapLabelSize(innerWidth, devicePixelRatio, sessionDevicePixelRatio) {
...
function page14MapLabelWeight(dprRatio, labelSize) {
...
function page14UseHorizontalColorbar(innerWidth, devicePixelRatio, sessionDevicePixelRatio) {
    const session = sessionDevicePixelRatio > 0 ? sessionDevicePixelRatio : 1;
    const dpr = devicePixelRatio > 0 ? devicePixelRatio : 1;
    const dprRatio = dpr / session;
...
function page14ColorbarFontSizes(labelSize, innerWidth, devicePixelRatio, sessionDevicePixelRatio) {
    const session = sessionDevicePixelRatio > 0 ? sessionDevicePixelRatio : 1;
    const dpr = devicePixelRatio > 0 ? devicePixelRatio : 1;
    const dprRatio = dpr / session;
...
const PAGE14_EXTERNAL_LABEL_CODES = new Set(['VT', 'ME', 'NH', 'MA', 'RI', 'CT', 'NJ', 'DE', 'MD', 'DC']);
 ...
 */
const PAGE14_OUTSIDE_LABEL_GEO = {
...
const PAGE14_INTERNAL_LABEL_POS = {
    ID: { lat: 43.88, lon: -115.35 },
};

const PAGE14_EXTERNAL_ORDER = ['ME', 'VT', 'NH', 'MA', 'RI', 'CT', 'NJ', 'DE', 'MD', 'DC'];

const PAGE14_ROWS_2024 = [
...
/** Add new years when data are available; map year → row list (same shape as PAGE14_ROWS_2024). */
const PAGE14_DATA_BY_YEAR = {
    2024: PAGE14_ROWS_2024,
};

const PAGE14_YEAR_LIST = Object.keys(PAGE14_DATA_BY_YEAR)
    .map(Number)
    .sort((a, b) => b - a);
...
const Page14 = () => {
    ...
    const [selectedYear, setSelectedYear] = useState(PAGE14_YEAR_LIST[0] ?? 2024);
    ...
    const scrollToFootnote = (e) => {
        e.preventDefault();
        document.getElementById('fn-asterisk-page14')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    };

    const scrollToRef = (e) => {
        e.preventDefault();
        document.getElementById('fn-asterisk-rf-page14')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    };
    ...
    const page14YearRange = useMemo(() => {
        if (!PAGE14_YEAR_LIST.length) return { min: 2024, max: 2024 };
        return { min: Math.min(...PAGE14_YEAR_LIST), max: Math.max(...PAGE14_YEAR_LIST) };
    }, []);

    const tableExportStem = useMemo(() => {
        const base = stripHtml(getText('page14_title', lang).replace(/\(\{\{year\}\}\)/g, '').trim());
        return `${toFileSlug(base)}-${page14YearRange.min}-${page14YearRange.max}`;
    }, [lang, page14YearRange.min, page14YearRange.max]);
    ...
    const tableRowsForExport = useMemo(() => {
        ...
        const out = [];
        for (const y of [...PAGE14_YEAR_LIST].sort((a, b) => b - a)) {
            const yrData = PAGE14_DATA_BY_YEAR[y];
            if (!yrData) continue;
    ...
    const downloadChartWithTitle = useCallback(async () => {
        ...
        try {
            const title = stripHtml(getText('page14_title', lang).replace(/\{\{year\}\}/g, String(selectedYear)));
            const subtitle = stripHtml(getText('page14_subtitle', lang));
    ...
    const downloadTableAsCSV = useCallback(() => {
        const headers = [
            getText('page14_table_year', lang),
            getText('page14_table_state', lang),
            getText('page14_table_exports_mil', lang),
            getText('page14_table_pct_gdp', lang),
        ];
        const lines = [
            headers.join(','),
    ...
    const downloadTableAsDocx = useCallback(async () => {
        const yearInHeading =
            page14YearRange.min === page14YearRange.max
                ? String(page14YearRange.min)
                : `${page14YearRange.min}–${page14YearRange.max}`;
        const heading = stripHtml(getText('page14_title', lang).replace(/\{\{year\}\}/g, yearInHeading));
        const yLabel = getText('page14_table_year', lang);
        const sLabel = getText('page14_table_state', lang);
        const eLabel = getText('page14_table_exports_mil', lang);
        const pLabel = getText('page14_table_pct_gdp', lang);
    ...
    const page14ColorbarHorizontal = useMemo(
        () => page14UseHorizontalColorbar(windowWidth, devicePixelRatio, sessionDevicePixelRatio),
        [windowWidth, devicePixelRatio, sessionDevicePixelRatio]
    );

    const plotBundle = useMemo(() => {
        const rows = PAGE14_DATA_BY_YEAR[selectedYear] || PAGE14_ROWS_2024;
        const fmt = (n) =>
        ...
        const labelSize = page14MapLabelSize(windowWidth, devicePixelRatio, sessionDevicePixelRatio);
        const mapLabelWeight = page14MapLabelWeight(
        ...
        const pctTail = getText('page14_hover_pct_tail', lang);
        ...
        const labelText = rows.map((r) => {
            if (PAGE14_EXTERNAL_LABEL_CODES.has(r.code)) return '';
        ...
        const leaderLat = [];
        const leaderLon = [];
        for (const code of PAGE14_EXTERNAL_ORDER) {
            const row = rows.find((r) => r.code === code);
            const g = PAGE14_OUTSIDE_LABEL_GEO[code];
            if (!row || !g) continue;
        ...
        const useHorizontalColorbar = page14UseHorizontalColorbar(
            windowWidth,
            devicePixelRatio,
            sessionDevicePixelRatio
        );
        const cbFonts = page14ColorbarFontSizes(
            labelSize,
            windowWidth,
            devicePixelRatio,
            sessionDevicePixelRatio
        ...
        const colorbarTitleText = getText('page14_colorbar_title', lang);
        ...
        const labelColors = rows.map((r, i) => {
            if (PAGE14_EXTERNAL_LABEL_CODES.has(r.code)) return 'rgba(0,0,0,0)';
            const o = opacities[i];
        ...
        const scatterInternal = {
            type: 'scattergeo',
            mode: 'text',
            lat: rows.map((r) => PAGE14_INTERNAL_LABEL_POS[r.code]?.lat ?? r.lat),
            lon: rows.map((r) => PAGE14_INTERNAL_LABEL_POS[r.code]?.lon ?? r.lon),
            text: labelText,
            textfont: {
                ...mapLabelFont,
                color: labelColors,
    ...
    return (
        <main
            ...
            role="main"
            aria-labelledby="page14-chart-heading"
            style={{
        ...
        >
            <style>{`
.page-14 { width: 100%; }
.page14-container { width: 100%; padding: 15px 0 0 0; box-sizing: border-box; }
.page14-year-selector label { display: block; font-size: 14px; font-weight: bold; margin-bottom: 5px; font-family: 'Noto Sans', sans-serif; }
.page14-year-selector .page14-year-button:hover { border-color: #007bff; }
.page14-year-selector .page14-year-button:focus { outline: 2px solid #005fcc; outline-offset: 2px; border-color: #007bff; }
.page14-chart-frame {
...
.page14-map-wrap { width: 100%; position: relative; min-height: 480px; }
.page14-map-wrap .js-plotly-plot .plotly .modebar { right: 20px !important; }
.page14-table-wrapper { display: block; width: 100%; margin: 20px 0 0 0; }
.page14-table-wrapper details > summary {
    display: block;
    width: 100%;
    padding: 12px 15px;
...
.page14-table-wrapper details > summary::-webkit-details-marker { display: none; }
.page14-table-wrapper details > summary:hover { background-color: #404040 !important; }
.page14-table-wrapper button[type="button"]:hover,
.page14-table-wrapper button:hover,
.page14-chart-frame button[type="button"]:hover,
.page14-chart-frame button:hover { background-color: #404040 !important; }
.page14-table-wrapper .table-responsive { display: block; width: 100%; overflow-x: auto; border: 1px solid #ddd; background: #fff; }
.page14-table-wrapper .table-responsive table { width: max-content; min-width: 100%; border-collapse: collapse; }
@media (max-width: 1097px) {
    .page14-chart-subtitle { margin-left: 60px; }
}
@media (max-width: 960px) {
    .page14-chart-subtitle { margin-left: 20px; }
}
@media (max-width: 768px) {
    .page14-chart-title { font-size: 26px; }
    .page14-chart-subtitle { font-size: 18px; }
}
            `}</style>
            <div className="page14-container">
                <div ref={yearDropdownRef} className="page14-year-selector" style={{ position: 'relative', marginBottom: '20px', width: '200px' }}>
                    <label htmlFor="page14-year-button">{getText('year_slider_label', lang)}</label>
                    <button
                        id="page14-year-button"
                        ref={yearButtonRef}
                        type="button"
                        className="page14-year-button"
                    ...
                    {isYearDropdownOpen && (
                        ...
                        >
                            {[...PAGE14_YEAR_LIST].sort((a, b) => b - a).map((y) => {
                ...
                <div className="page14-chart-frame">
                    <h2 id="page14-chart-heading" className="page14-chart-title">
                        {getText('page14_title', lang).replace(/\{\{year\}\}/g, String(selectedYear))}
                        <sup
                            id="fn-asterisk-rf-page14"
                            style={{ verticalAlign: 'super', fontSize: '0.6em', lineHeight: 0 }}
                        >
                            <a href="#fn-asterisk-page14" onClick={scrollToFootnote} className="fn-lnk">
                                <span className="wb-inv">{lang === 'en' ? 'Footnote ' : 'Note de bas de page '}</span>
                                <span aria-hidden="true">*</span>
                            </a>
                        </sup>
                    </h2>
                    <p className="page14-chart-subtitle">{getText('page14_subtitle', lang)}</p>
                    {selectedPoints !== null && (
                    ...
                    <div
                        className="page14-map-wrap"
                        role="region"
                        aria-label={getText('page14_map_aria', lang)}
                        tabIndex="0"
                    >
                        <figure ref={chartRef} style={{ margin: 0, position: 'relative', width: '100%' }}>
                            <Plot
                                key={page14ColorbarHorizontal ? 'page14-map-cb-h' : 'page14-map-cb-v'}
                    ...
                    <div className="page14-table-wrapper">
                            ...
                            >
                                <table className="table table-striped table-hover">
                                    <caption className="wb-inv">
                                        {stripHtml(
                                            getText('page14_title', lang).replace(
                                                /\{\{year\}\}/g,
                                                page14YearRange.min === page14YearRange.max
                                                    ? String(page14YearRange.min)
                                                    : `${page14YearRange.min}–${page14YearRange.max}`
                ...
                <aside className="wb-fnote" role="note">
                    <h2 id="page14-fn-heading">{lang === 'en' ? 'Footnotes' : 'Notes de bas de page'}</h2>
                    <dl>
                        <dt>{lang === 'en' ? 'Footnote *' : 'Note de bas de page *'}</dt>
                        <dd id="fn-asterisk-page14">
                            <a
                                href="#fn-asterisk-rf-page14"
                                onClick={scrollToRef}
                                className="fn-num"
                                title={
                                    lang === 'en'
                            ...
                            <p>{getText('page14_footnote', lang)}</p>
                        </dd>
                    </dl>
                </aside>
            </div>
