...
const Page81 = () => {
    ...
    const getProvLabel = useCallback((key) => getText(`page67_prov_${key}`, lang), [lang]);
    const getLegendLabel = useCallback((key) => getText(`page81_legend_${key}`, lang), [lang]);
    ...
    const scrollToFootnote = (e) => {
        e.preventDefault();
        document.getElementById('fn-asterisk-page81')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    };

    const scrollToRef = (e) => {
        e.preventDefault();
        document.getElementById('fn-asterisk-rf-page81')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    };
    ...
    const togglePointSelection = (pointIndex, setSelected, lastClickRef) => {
    ...
    const useSyncedTableScroll = (isOpen, topRef, tableRef, bottomRef) => {
        ...
        }, [isOpen, topRef, tableRef, bottomRef, syncTableScroll]);
    };
    ...
    const useVerticalProvTicks = zoomEffective >= PAGE81_PROV_VERTICAL_TICK_ZOOM || windowWidth <= 480;
    const useWrapProjLabels = zoomEffective >= PAGE81_PROJ_WRAP_ZOOM;
    const projLabelMaxLength = zoomEffective >= PAGE81_PROJ_WRAP_DEEP_ZOOM ? 14 : 20;
    ...
    }, [projects.length, projTickText, useWrapProjLabels, projRowHeight]);
    ...
    }], [provLabels, provValues, provinces, selectedProvPoints, barLabelFont, locale]);
    ...
    }], [projLabels, projValues, projects, projBaseColors, selectedProjPoints, barLabelFont, locale]);
    ...
    const projPlotLayout = useMemo(() => ({
        ...
        plot_bgcolor: 'rgba(0,0,0,0)',
        autosize: true,
    }), [projXMax, projLabels, projTickText, tickFont, windowWidth, projPlotHeight, useWrapProjLabels, projLeftMargin, projRightMargin]);
    ...
    const onProvPlotReady = useCallback(
        (_figure, graphDiv) => bindPlotClickHandler(graphDiv, provClickHandlerRef, '_page81ProvClick'),
        [bindPlotClickHandler],
    );

    const onProjPlotReady = useCallback(
        (_figure, graphDiv) => bindPlotClickHandler(graphDiv, projClickHandlerRef, '_page81ProjClick'),
        [bindPlotClickHandler],
    );
    ...
    const downloadProjChartWithLegend = async () => {
        const plotElement = projChartRef?.current?.querySelector('.js-plotly-plot');
        if (!plotElement || !window.Plotly) return;
        try {
            const title = `${stripHtml(getText('page81_proj_chart_title', lang))}*${stripHtml(getText('page81_proj_chart_title_suffix', lang))}`;
    ...
    const downloadProvCsv = () => {
        const headers = [getText('page81_table_col_province', lang), getText('page81_table_col_capacity', lang)];
    ...
    const downloadProjCsv = () => {
        const headers = [getText('page81_table_col_facility', lang), getText('page81_table_col_total_capacity', lang)];
    ...
    const downloadProvDocx = async () => {
        const title = stripHtml(getText('page81_prov_chart_title', lang));
        const headerCells = [
            new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: getText('page81_table_col_province', lang), bold: true, size: 22 })], alignment: AlignmentType.CENTER })], shading: { fill: 'E6E6E6' } }),
            new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: getText('page81_table_col_capacity', lang), bold: true, size: 22 })], alignment: AlignmentType.CENTER })], shading: { fill: 'E6E6E6' } }),
        ];
    ...
    const downloadProjDocx = async () => {
        const title = `${stripHtml(getText('page81_proj_chart_title', lang))}*${stripHtml(getText('page81_proj_chart_title_suffix', lang))}`;
        const headerCells = [
            new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: getText('page81_table_col_facility', lang), bold: true, size: 22 })], alignment: AlignmentType.LEFT })], shading: { fill: 'E6E6E6' } }),
            new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: getText('page81_table_col_total_capacity', lang), bold: true, size: 22 })], alignment: AlignmentType.CENTER })], shading: { fill: 'E6E6E6' } }),
        ];
    ...
    const provChartConfig = useMemo(() => ({
        ...
        modeBarButtonsToAdd: [{
            name: getText('page81_download_prov_png', lang),
    ...
    const projChartConfig = useMemo(() => ({
        ...
        modeBarButtonsToRemove: MODEBAR_REMOVE,
        modeBarButtonsToAdd: [{
            name: getText('page81_download_proj_png', lang),
            icon: { width: 24, height: 24, path: 'M13 8V2H7v6H2l8 8 8-8h-5zM0 18h20v2H0v-2z' },
            click: () => downloadProjChartWithLegend(),
        }],
    }), [lang, projPlotHeight]);

    const provTableCaption = substitute(getText('page81_table_caption_prov', lang), { year: referenceYear ?? '' });
    const projTableCaption = substitute(getText('page81_table_caption_proj', lang), { year: referenceYear ?? '' });
    ...
    const renderDataTable = ({
        ...
        <div className="page81-table-wrapper">
            <details className="page81-data-table" onToggle={(e) => setIsOpen(e.currentTarget.open)}>
                ...
                <div ref={topRef} className="page81-table-scrollbar" aria-hidden="true"><div /></div>
                <div
                    ref={tableRef}
                    className="page81-table-responsive"
                    role="region"
                ...
                <div ref={bottomRef} className="page81-table-scrollbar" aria-hidden="true"><div /></div>
                <div className="page81-download-buttons">
                    <button type="button" onClick={onCsv}>
                        {getText('page81_download_csv', lang)}
                    </button>
                    <button type="button" onClick={onDocx}>
                        {getText('page81_download_docx', lang)}
                    </button>
                </div>
            </details>
        </div>
    ...
    return (
        <main
            ...
            role="main"
            aria-labelledby="page81-prov-chart-title"
            style={{ backgroundColor: '#ffffff' }}
        >
            <style>{`
.page-81 { width: 100%; }
.page81-container { width: 100%; padding: 15px 0 40px 0; display: flex; flex-direction: column; box-sizing: border-box; }
.page81-chart-frame { background-color: #f5f5f5; padding: 20px; border-radius: 8px; margin-bottom: 24px; box-sizing: border-box; overflow: visible; }
.page81-chart-title { font-family: 'Lato', sans-serif; font-size: 29px; font-weight: bold; color: var(--gc-text); text-align: center; margin: 0 0 12px 0; text-transform: none; }
.page81-chart-title .fn-lnk { color: #26374a; text-decoration: underline; }
.page81-chart-scroll { width: 100%; overflow: visible; position: relative; }
.page81-chart { width: 100%; min-width: 0; position: relative; z-index: 1; overflow: visible; }
.page81-chart > div { width: 100%; height: 100%; overflow: visible; }
.page81-chart .js-plotly-plot,
.page81-chart .plot-container,
.page81-chart .svg-container { overflow: visible !important; pointer-events: auto !important; }
.page81-chart .js-plotly-plot .plotly .modebar { right: 4px !important; top: 2px !important; }
.page81-clear-selection { padding: 6px 12px; background-color: #8C8C8C; border: 1px solid #404040; border-radius: 4px; cursor: pointer; font-family: Arial, sans-serif; font-size: 14px; color: #fff; margin-bottom: 8px; }
.page81-proj-column-headers {
    display: grid;
...
.page81-proj-column-headers span:first-child {
    text-align: right;
    padding-right: 2px;
    box-sizing: border-box;
}
.page81-proj-column-headers span:last-child {
    text-align: right;
    box-sizing: border-box;
}
.page81-legend { display: flex; flex-wrap: wrap; justify-content: center; gap: 18px 28px; margin-top: 16px; font-family: Arial, sans-serif; font-size: 14px; color: var(--gc-text); }
.page81-legend-item { display: inline-flex; align-items: center; gap: 8px; white-space: nowrap; }
.page81-legend-swatch { width: 22px; height: 14px; display: inline-block; border: 1px solid rgba(0, 0, 0, 0.12); }
.page81-table-wrapper { display: block; width: 100%; margin-top: 20px; margin-bottom: 0; }
.page81-table-wrapper details > summary { display: block; width: 100%; padding: 12px 15px; background-color: #8C8C8C; border: 1px solid #404040; border-radius: 4px; cursor: pointer; font-family: Arial, sans-serif; font-weight: bold; color: #ffffff; list-style: none; }
.page81-table-wrapper details > summary::-webkit-details-marker { display: none; }
.page81-table-scrollbar { width: 100%; overflow-x: auto; overflow-y: hidden; margin: 0; }
...
.page81-footnotes { font-family: var(--font-body); font-size: 1rem; color: var(--gc-text); margin-top: 24px; margin-bottom: 0; padding-top: 12px; border-top: 1px solid #e0e0e0; line-height: 1.65; max-width: 100%; box-sizing: border-box; }
.page81-footnotes h2 { font-family: var(--font-heading); font-size: 1.4rem; font-weight: 700; color: var(--gc-text); margin-top: 0; margin-bottom: 1rem; }
.page81-footnotes dd { display: flex; align-items: flex-start; gap: 0.75rem; margin-left: 0; margin-bottom: 1rem; }
.page81-loading, .page81-error { font-family: 'Noto Sans', sans-serif; font-size: 18px; color: var(--gc-text); margin: 24px 0; }
@media (max-width: 768px) {
    .page81-chart-title { font-size: 26px; }
    .page81-footnotes { font-size: 0.9rem; }
    .page81-footnotes h2 { font-size: 1.2rem; margin-bottom: 0.75rem; }
    .page81-proj-column-headers { font-size: 14px; }
}
            `}</style>

            <div className="page81-container">
                {loading && <p className="page81-loading">{lang === 'en' ? 'Loading data…' : 'Chargement des données…'}</p>}
                {error && <p className="page81-error" role="alert">{error}</p>}
                ...
                {!loading && !error && provinces.length > 0 && (
                    <div className="page81-chart-frame">
                        ...
                        {selectedProvPoints !== null && (
                            <button type="button" className="page81-clear-selection" onClick={clearProvSelection}>
                                {lang === 'en' ? 'Clear selection' : 'Effacer la sélection'}
                            </button>
                        )}

                        <div className="page81-chart-scroll">
                            <figure
                                ref={provChartRef}
                            ...
                            >
                                <Plot
                                    key={`page81-prov-${useVerticalProvTicks ? 'v' : 'h'}-${provPlotHeight}`}
                                    data={provPlotData}
                        ...
                        {renderDataTable({
                            ...
                            captionId: 'page81-prov-table-caption',
                            headers: [getText('page81_table_col_province', lang), getText('page81_table_col_capacity', lang)],
                ...
                {!loading && !error && projects.length > 0 && (
                    <div className="page81-chart-frame">
                        <h2 id="page81-proj-chart-title" className="page81-chart-title">
                            {getText('page81_proj_chart_title', lang)}
                            <span id="fn-asterisk-rf-page81" style={{ verticalAlign: 'super', fontSize: '0.75em' }}>
                                <a className="fn-lnk" href="#fn-asterisk-page81" onClick={scrollToFootnote}>
                        ...
                        {selectedProjPoints !== null && (
                            <button type="button" className="page81-clear-selection" onClick={clearProjSelection}>
                                {lang === 'en' ? 'Clear selection' : 'Effacer la sélection'}
                            </button>
                        )}

                        <div className="page81-proj-column-headers" style={projColumnHeaderStyle} aria-hidden="true">
                            <span>{getText('page81_table_col_facility', lang)}</span>
                            <span>{getText('page81_table_col_total_capacity', lang)}</span>
                        </div>
                        ...
                        <div className="page81-chart-scroll">
                            <figure
                                ...
                                role="region"
                                aria-label={`${getText('page81_proj_chart_title', lang)}${getText('page81_proj_chart_title_suffix', lang)}`}
                                tabIndex={0}
                            >
                                <Plot
                                    key={`page81-proj-${useWrapProjLabels ? 'wrap' : 'plain'}-${projPlotHeight}-${projLeftMargin}`}
                                    data={projPlotData}
                        ...
                        <div className="page81-legend" aria-hidden="false">
                            {LEGEND_KEYS.map((key) => (
                                <span key={key} className="page81-legend-item">
                                    <span className="page81-legend-swatch" style={{ backgroundColor: PROJECT_PROV_COLORS[key] }} />
                                    {getLegendLabel(key)}
                                </span>
                            ))}
                        </div>
                        ...
                        {renderDataTable({
                            ...
                            captionId: 'page81-proj-table-caption',
                            headers: [getText('page81_table_col_facility', lang), getText('page81_table_col_total_capacity', lang)],
                ...
                {!loading && !error && projects.length > 0 && (
                    <aside className="wb-fnote page81-footnotes" role="note">
                        <h2>{lang === 'en' ? 'Footnotes' : 'Notes de bas de page'}</h2>
                        <dl>
                            <dt className="wb-inv">{lang === 'en' ? 'Footnote' : 'Note de bas de page'}</dt>
                            <dd id="fn-asterisk-page81">
                                <a href="#fn-asterisk-rf-page81" onClick={scrollToRef} className="fn-num" title={lang === 'en' ? 'Return to footnote * referrer' : 'Retour à la référence de la note de bas de page *'}>
                                    <span className="wb-inv">{lang === 'en' ? 'Return to footnote ' : 'Retour à la note de bas de page '}</span>*
                                </a>
                                {' '}{getText('page81_footnote', lang)}
                            </dd>
                        </dl>
                    </aside>
                )}
