...
const Page46FilterDropdown = ({ label, filterValues, options, isOpen, setOpen, setFilter, closeOthers, lang }) => {
    ...
    return (
        <div className="page46-filter-dropdown" style={FILTER_DROPDOWN_WRAP}>
...
const Page46 = () => {
    ...
    const getLocationLabel = useCallback((key) => getText(`page46_location_${key}`, lang), [lang]);
    const getRelianceLabel = useCallback((key) => getText(`page46_reliance_${key}`, lang), [lang]);
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            if (event.target.closest('.page46-filter-dropdown') || event.target.closest('.page46-filter-panel')) return;
    ...
    const barLabels = useMemo(() => PAGE46_BAR_PROVINCES.map(p => getProvinceName(p.key)), [getProvinceName]);
    const barValues = useMemo(() => PAGE46_BAR_PROVINCES.map(p => p.count), []);

    const barTrace = useMemo(() => {
        const hovertext = PAGE46_BAR_PROVINCES.map((p, i) => {
            const name = getProvinceName(p.key);
            return `<b>${name}</b><br>${stripHtml(getText('page46_bar_chart_title', lang))}: ${barValues[i]}`;
        });
        const markerColor = selectedBarPoints === null
            ? BAR_COLOR
            : PAGE46_BAR_PROVINCES.map((_, i) => selectedBarPoints.includes(i) ? BAR_COLOR : hexToRgba(BAR_COLOR, 0.3));
        return {
            type: 'bar',
            orientation: 'h',
            x: barValues,
    ...
    const plotlyMapData = useMemo(() => {
        ...
        const scatterTrace = {
            ...
            hovertext: filteredCommunities.map(c =>
                ...
                `${getText('page46_filter_province', lang)}: ${getProvinceName(c.province)}<br>` +
                `${getText('page46_filter_location', lang)}: ${getLocationLabel(c.location)}<br>` +
                `${getText('page46_filter_reliance', lang)}: ${getRelianceLabel(c.reliance)}`
            ),
    ...
    const downloadMapWithTitleAndLegend = async () => {
        ...
        if (!plotElement || !window.Plotly) return;
        try {
            const title = stripHtml(getText('page46_map_title', lang));
    ...
    const barConfig = {
        ...
        modeBarButtonsToAdd: [{
            ...
            click: () => downloadChartWithTitle(barChartRef, 'page46_bar_chart_title', 'energy_reliant_communities_by_province.png', 'communautes_dependantes_energie_par_province.png')
        }]
    };
    ...
    const downloadBarCSV = () => {
        const headers = [getText('page46_table_province', lang), getText('page46_table_count', lang)];
        const rows = PAGE46_BAR_PROVINCES.map(p => [getProvinceName(p.key), p.count]);
    ...
    const downloadBarDocx = async () => {
        const title = stripHtml(getText('page46_title', lang));
        const headerCells = [
            new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: getText('page46_table_province', lang), bold: true, size: 22 })], alignment: AlignmentType.CENTER })], shading: { fill: 'E6E6E6' } }),
            new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: getText('page46_table_count', lang), bold: true, size: 22 })], alignment: AlignmentType.CENTER })], shading: { fill: 'E6E6E6' } })
        ];
        const dataRows = PAGE46_BAR_PROVINCES.map(p => new TableRow({
            children: [
    ...
    const downloadMapCSV = () => {
        const headers = [
            getText('page46_table_community', lang),
            getText('page46_table_province', lang),
            getText('page46_filter_location', lang),
            getText('page46_filter_reliance', lang)
        ];
    ...
    const getMapChartSummary = () => {
        return `${stripHtml(getText('page46_map_title', lang))}. ${filteredCommunities.length} ${lang === 'en' ? 'communities shown.' : 'communautés affichées.'}`;
    };
    ...
    return (
        <main
            ...
            role="main"
            aria-labelledby="page46-title"
            style={{ backgroundColor: '#ffffff', overflow: 'visible' }}
        >
            <style>{`
.page-46 { width: 100%; overflow: visible !important; }
.page46-container { width: 100%; padding: 15px 0 40px 0; display: flex; flex-direction: column; box-sizing: border-box; overflow: visible; }
.page46-title {
...
.page46-bar-chart { width: 100%; height: 420px; }
.page46-map-chart { width: 100%; overflow: hidden; position: relative; }
.page46-filter-region { position: relative; overflow: visible; margin-bottom: 0; }
.page46-filter-dropdowns { display: flex; flex-wrap: wrap; gap: 10px; overflow: visible; position: relative; }
.page46-map-legend {
    display: flex;
    flex-wrap: wrap;
...
.page46-table-wrapper { display: block; width: 100%; margin: 20px 0 0 0; }
.page46-table-wrapper details > summary {
    display: block;
    width: 100%;
    padding: 12px 15px;
...
.page46-table-wrapper details > summary::-webkit-details-marker { display: none; }
.page46-table-wrapper details > summary:hover,
.page46-table-wrapper button[type="button"]:hover,
.page46-chart-frame button[type="button"]:hover { background-color: #404040 !important; }
.page46-table-wrapper .table-responsive { display: block; width: 100%; overflow-x: auto; border: 1px solid #ddd; background: #fff; }
.page46-table-wrapper .table-responsive table { width: max-content; min-width: 100%; border-collapse: collapse; }
@media (max-width: 768px) {
    .page46-title { font-size: 37px; }
    .page46-chart-title, .page46-map-title { font-size: 26px; }
    .page46-intro { font-size: 18px; }
    .page46-bar-chart { height: 380px; }
}
            `}</style>

            <div className="page46-container">
                <header role="region" aria-label={getText('page46_title', lang)}>
                    <h1 id="page46-title" className="page46-title">{getText('page46_title', lang)}</h1>
                </header>

                <p className="page46-intro" role="region" aria-label={stripHtml(getText('page46_para1', lang))}>
                    {getText('page46_para1', lang)}
                </p>
                <p className="page46-intro" role="region" aria-label={stripHtml(getText('page46_para2_part1', lang) + getText('page46_para2_bold1', lang) + getText('page46_para2_part2', lang) + getText('page46_para2_bold2', lang))}>
                    {getText('page46_para2_part1', lang)}<strong>{getText('page46_para2_bold1', lang)}</strong>{getText('page46_para2_part2', lang)}<strong>{getText('page46_para2_bold2', lang)}</strong>
                </p>

                <div className="page46-chart-frame">
                    <h2 className="page46-chart-title">{getText('page46_bar_chart_title', lang)}</h2>
                    <div role="region" aria-label={getBarChartSummary()} tabIndex="0">
                        <figure ref={barChartRef} className="page46-bar-chart" style={{ margin: 0, position: 'relative' }}>
                    ...
                    <div className="page46-table-wrapper">
                        <details onToggle={(e) => setIsBarTableOpen(e.currentTarget.open)}>
                            ...
                            <div ref={barTableScrollRef} className="table-responsive" role="region" style={{ borderTop: 'none', padding: '15px' }} tabIndex="0">
                                <table className="table table-striped table-hover">
                                    <caption className="wb-inv">{getText('page46_bar_chart_title', lang)}</caption>
                                    <thead>
                                        <tr>
                                            <th scope="col" style={{ fontWeight: 'bold', padding: '10px', border: '1px solid #ddd' }}>{getText('page46_table_province', lang)}</th>
                                            <th scope="col" style={{ fontWeight: 'bold', padding: '10px', border: '1px solid #ddd', textAlign: 'center' }}>{getText('page46_table_count', lang)}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {PAGE46_BAR_PROVINCES.map(p => (
                                            <tr key={p.key}>
                ...
                <div className="page46-chart-frame">
                    ...
                    <div className="page46-map-legend" aria-hidden="true">
                        {relianceKeys.map(key => (
                            <div key={key} className="page46-legend-item">
                                <span className="page46-legend-dot" style={{ backgroundColor: RELIANCE_COLORS[key] }} />
                ...
                <div
                    className="page46-filter-region"
                    role="region"
                ...
                >
                    ...
                    <div className="page46-filter-dropdowns" style={{ marginBottom: '0', padding: '10px', backgroundColor: '#fafafa', borderRadius: '4px', border: '1px solid #e0e0e0' }}>
                        <Page46FilterDropdown
                            label={getText('page46_filter_community', lang)}
                        ...
                        />
                        <Page46FilterDropdown
                            label={getText('page46_filter_province', lang)}
                        ...
                        />
                        <Page46FilterDropdown
                            label={getText('page46_filter_location', lang)}
                        ...
                        />
                        <Page46FilterDropdown
                            label={getText('page46_filter_reliance', lang)}
