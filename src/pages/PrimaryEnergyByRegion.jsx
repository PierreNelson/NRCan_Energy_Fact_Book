...
const primary_energy_by_region_HARDCODED = {
    year: 2023,
    regions: {
...
const PrimaryEnergyByRegion = () => {
    const { lang } = useOutletContext();
    const pageData = primary_energy_by_region_HARDCODED;
    ...
    const regionLabels = useMemo(() =>
        PEP_REGIONS.map(r => getText(`primary_energy_by_region_region_${r}`, lang)),
        [lang]
    );
    ...
    const chart1Traces = useMemo(() => {
        ...
        return SOURCES_INCLUDING.map((sourceKey, traceIndex) => {
            const sourceLabel = getText(sourceKey === 'uranium' ? 'primary_energy_production_uranium' : `primary_energy_production_${sourceKey}`, lang);
            const xArr = PEP_REGIONS.map(region => pageData.regions[region]?.[sourceKey] ?? 0);
            const hovertext = PEP_REGIONS.map((region, i) => {
                const val = xArr[i];
                const regionLabelFull = getText(`primary_energy_by_region_region_${region}_full`, lang);
    ...
    const chart2Traces = useMemo(() => {
        ...
        return SOURCES_EXCLUDING.map((sourceKey, traceIndex) => {
            ...
            const sourceLabel = getText(`primary_energy_production_${sourceKey}`, lang);
    ...
    const plotLayout = (xMaxVal) => ({
        ...
        xaxis: {
            title: { text: getText('primary_energy_by_region_xaxis', lang), font: { size: windowWidth <= 480 ? 14 : 18, family: 'Arial, sans-serif' }, standoff: 15 },
            range: [0, xMaxVal],
            tick0: 0,
            dtick: 2000,
            showgrid: true,
    ...
    const downloadTableAsCSV1 = () => {
        const sourceLabels = SOURCES_INCLUDING.map(s => getText(s === 'uranium' ? 'primary_energy_production_uranium' : `primary_energy_production_${s}`, lang));
        ...
        const rows = PEP_REGIONS.map(region => {
            ...
            return [getText(`primary_energy_by_region_region_${region}`, lang), ...vals.map(v => formatProductionByRegionNumber(v, lang)), formatProductionByRegionNumber(total, lang)];
        });
    ...
    const downloadTableAsCSV2 = () => {
        const sourceLabels = SOURCES_EXCLUDING.map(s => getText(`primary_energy_production_${s}`, lang));
        const headers = [lang === 'en' ? 'Region' : 'Région', ...sourceLabels, lang === 'en' ? 'Total (PJ)' : 'Total (PJ)'];
        const rows = PEP_REGIONS.map(region => {
            const r = pageData.regions[region] || {};
    ...
    const downloadTableAsDocx1 = async () => {
        const title = stripHtml(getText('primary_energy_by_region_title', lang)) + ' – ' + stripHtml(getText('primary_energy_by_region_chart1_title', lang));
        const sourceLabels = SOURCES_INCLUDING.map(s => getText(s === 'uranium' ? 'primary_energy_production_uranium' : `primary_energy_production_${s}`, lang));
        ...
        const dataRows = PEP_REGIONS.map(region => {
            ...
            const vals = SOURCES_INCLUDING.map(s => r[s] ?? 0);
            const total = vals.reduce((a, b) => a + b, 0);
            const cells = [getText(`primary_energy_by_region_region_${region}`, lang), ...vals.map(v => formatProductionByRegionNumber(v, lang)), formatProductionByRegionNumber(total, lang)].map((text, i) =>
    ...
    const downloadTableAsDocx2 = async () => {
        const title = stripHtml(getText('primary_energy_by_region_title', lang)) + ' – ' + stripHtml(getText('primary_energy_by_region_chart2_title', lang));
        const sourceLabels = SOURCES_EXCLUDING.map(s => getText(`primary_energy_production_${s}`, lang));
        const headerCells = [lang === 'en' ? 'Region' : 'Région', ...sourceLabels, lang === 'en' ? 'Total (PJ)' : 'Total (PJ)'].map(h =>
            new TableCell({ children: [new Paragraph({ children: [new TextRun({ text: h, bold: true, size: 22 })], alignment: AlignmentType.CENTER })], shading: { fill: 'E6E6E6' } })
        );
    ...
    return (
        <main
            ...
            role="main"
            aria-labelledby="primary-energy-by-region-title"
            style={{ backgroundColor: '#ffffff' }}
        >
            <style>{`
.page-5 { width: 100%; }
.primary-energy-by-region-container { width: 100%; padding: 15px 0 0 0; display: flex; flex-direction: column; box-sizing: border-box; flex: 1; overflow: visible; }
.primary-energy-by-region-title { font-family: 'Lato', sans-serif; font-size: 41px; font-weight: bold; color: var(--gc-text, #332f30); margin-top: 0; margin-bottom: 25px; line-height: 1.2; position: relative; padding-bottom: 0.5em; }
.primary-energy-by-region-title::after { content: ''; position: absolute; left: 0; bottom: 0.2em; width: 72px; height: 6px; background-color: var(--gc-red, #A62A1E); }
.primary-energy-by-region-chart-frame { background-color: #f5f5f5; padding: 20px; border-radius: 8px; margin-bottom: 20px; box-sizing: border-box; overflow: visible; }
.primary-energy-by-region-chart-title { font-family: 'Lato', sans-serif; font-size: 29px; font-weight: bold; color: #333333; margin-top: 0; margin-bottom: 10px; text-align: center; }
.primary-energy-by-region-chart { width: 100%; height: 420px; position: relative; z-index: 1; }
.primary-energy-by-region-chart button[type="button"]:hover,
.primary-energy-by-region-chart button:hover,
.primary-energy-by-region-chart-frame button[type="button"]:hover,
.primary-energy-by-region-chart-frame button:hover { background-color: #404040 !important; }
.primary-energy-by-region-legend { display: flex; flex-wrap: wrap; justify-content: center; gap: 20px 24px; margin-top: 16px; margin-bottom: 10px; font-family: Arial, sans-serif; position: relative; z-index: 10; background-color: #f5f5f5; padding: 10px 0; }
.primary-energy-by-region-legend-item { display: flex; align-items: center; gap: 8px; }
.primary-energy-by-region-legend-color { width: 20px; height: 12px; display: inline-block; }
.primary-energy-by-region-legend-label { font-size: 18px; color: #333333; }
.primary-energy-by-region-no-data { font-family: 'Noto Sans', sans-serif; font-size: 18px; color: #555; padding: 24px; text-align: center; }
.primary-energy-by-region-table-wrapper { display: block; width: 100%; margin: 20px 0 0 0; }
.primary-energy-by-region-table-wrapper details > summary { display: block; width: 100%; padding: 12px 15px; background-color: #8C8C8C; border: 1px solid #404040; border-radius: 4px; cursor: pointer; font-family: Arial, sans-serif; font-weight: bold; color: #ffffff; list-style: none; }
.primary-energy-by-region-table-wrapper details > summary::-webkit-details-marker { display: none; }
.primary-energy-by-region-table-wrapper details > summary:hover { background-color: #404040 !important; }
.primary-energy-by-region-table-wrapper button[type="button"]:hover,
.primary-energy-by-region-table-wrapper button:hover { background-color: #404040 !important; }
.primary-energy-by-region-table-wrapper .table-responsive { display: block; width: 100%; overflow-x: auto; border: 1px solid #ddd; background: #fff; }
.primary-energy-by-region-table-wrapper .table-responsive table { width: max-content; min-width: 100%; border-collapse: collapse; }
@media (max-width: 768px) {
.primary-energy-by-region-title { font-size: 37px; }
.primary-energy-by-region-chart-title { font-size: 26px; }
.primary-energy-by-region-legend-label { font-size: 14px; }
}
            `}</style>
            <div className="primary-energy-by-region-container">
                <header role="region" aria-label={getText('primary_energy_by_region_title', lang)}>
                    <h1 id="primary-energy-by-region-title" className="primary-energy-by-region-title" aria-hidden="true">{getText('primary_energy_by_region_title', lang)}</h1>
                    <h1 className="wb-inv">{stripHtml(getText('primary_energy_by_region_title', lang))}</h1>
                </header>

                <div className="primary-energy-by-region-chart-frame">
                    <h2 className="primary-energy-by-region-chart-title">{getText('primary_energy_by_region_chart1_title', lang)}</h2>
                    {hasData ? (
                        <>
                            <div role="region" aria-label={getText('primary_energy_by_region_chart1_title', lang)} tabIndex="0">
                                {selectedPoints1 !== null && selectedPoints1.length > 0 && (
                                    ...
                                    <figure ref={chartRef1} className="primary-energy-by-region-chart" style={{ margin: 0, position: 'relative' }}>
                                    <div aria-hidden="true">
                                        <Plot
                                            data={chart1Traces}
                                            layout={plotLayout(xMax)}
                                            config={{ ...config, modeBarButtonsToAdd: [{ name: lang === 'en' ? 'Download chart as PNG' : 'Télécharger le graphique en PNG', icon: { width: 24, height: 24, path: 'M13 8V2H7v6H2l8 8 8-8h-5zM0 18h20v2H0v-2z' }, click: () => downloadChartWithTitle(chartRef1, 'primary_energy_by_region_chart1_title') }] }}
                            ...
                            <div className="primary-energy-by-region-legend" aria-hidden="true">
                                {SOURCES_INCLUDING.map(key => (
                                    <div key={key} className="primary-energy-by-region-legend-item">
                                        <span className="primary-energy-by-region-legend-color" style={{ backgroundColor: COLORS[key] }} />
                                        <span className="primary-energy-by-region-legend-label">{getText(`primary_energy_production_${key}`, lang)}</span>
                                    </div>
                                ))}
                            </div>
                            <div className="primary-energy-by-region-table-wrapper">
                                <details onToggle={(e) => setIsTableOpen1(e.currentTarget.open)}>
                                    ...
                                    <div ref={tableScrollRef1} className="table-responsive" role="region" style={{ borderTop: 'none', padding: '15px' }} tabIndex="0">
                                        <table className="table table-striped table-hover">
                                            <caption className="wb-inv">{getText('primary_energy_by_region_chart1_title', lang)}</caption>
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th scope="col" colSpan={8} style={{ fontWeight: 'bold', padding: '10px', border: '1px solid #ddd', textAlign: 'center' }}>{getText('primary_energy_by_region_xaxis', lang)}</th>
                                                </tr>
                                                <tr>
                                                    {SOURCES_INCLUDING.map(s => (
                                                        <th key={s} scope="col" style={{ fontWeight: 'bold', padding: '10px', border: '1px solid #ddd' }}>{getText(s === 'uranium' ? 'primary_energy_production_uranium' : `primary_energy_production_${s}`, lang)}</th>
                                                    ))}
                                            ...
                                            <tbody>
                                                {PEP_REGIONS.map(region => {
                                                    ...
                                                    return (
                                                        <tr key={region}>
                                                            <th scope="row" style={{ fontWeight: 'bold', padding: '8px', border: '1px solid #ddd' }}>{getText(`primary_energy_by_region_region_${region}`, lang)}</th>
                                                            {vals.map((v, i) => (
                                                                <td key={i} style={{ textAlign: 'center', padding: '8px', border: '1px solid #ddd' }} aria-label={`${getText(`primary_energy_by_region_region_${region}`, lang)}, ${getText(SOURCES_INCLUDING[i] === 'uranium' ? 'primary_energy_production_uranium' : `primary_energy_production_${SOURCES_INCLUDING[i]}`, lang)}: ${formatProductionByRegionNumber(v, lang)} PJ`}>{formatProductionByRegionNumber(v, lang)}</td>
                                                            ))}
                        ...
                        <p className="primary-energy-by-region-no-data">{lang === 'en' ? 'No data available.' : 'Aucune donnée disponible.'}</p>
                    )}
                </div>

                <div className="primary-energy-by-region-chart-frame">
                    <h2 className="primary-energy-by-region-chart-title">{getText('primary_energy_by_region_chart2_title', lang)}</h2>
                    {hasData ? (
                        <>
                            <div role="region" aria-label={getText('primary_energy_by_region_chart2_title', lang)} tabIndex="0">
                                {selectedPoints2 !== null && selectedPoints2.length > 0 && (
                                    ...
                                    <figure ref={chartRef2} className="primary-energy-by-region-chart" style={{ margin: 0, position: 'relative' }}>
                                    <div aria-hidden="true">
                                        <Plot
                                            data={chart2Traces}
                                            layout={plotLayout(xMax)}
                                            config={{ ...config, modeBarButtonsToAdd: [{ name: lang === 'en' ? 'Download chart as PNG' : 'Télécharger le graphique en PNG', icon: { width: 24, height: 24, path: 'M13 8V2H7v6H2l8 8 8-8h-5zM0 18h20v2H0v-2z' }, click: () => downloadChartWithTitle(chartRef2, 'primary_energy_by_region_chart2_title') }] }}
                                            style={{ width: '100%', height: '100%' }}
                                            useResizeHandler={true}
                            ...
                            <div className="primary-energy-by-region-table-wrapper">
                                <details onToggle={(e) => setIsTableOpen2(e.currentTarget.open)}>
                                    ...
                                    <div ref={tableScrollRef2} className="table-responsive" role="region" style={{ borderTop: 'none', padding: '15px' }} tabIndex="0">
                                        <table className="table table-striped table-hover">
                                            <caption className="wb-inv">{getText('primary_energy_by_region_chart2_title', lang)}</caption>
                                            <thead>
                                                <tr>
                                                    <th scope="col" rowSpan={2} style={{ fontWeight: 'bold', padding: '10px', border: '1px solid #ddd', verticalAlign: 'middle' }}>{lang === 'en' ? 'Region' : 'Région'}</th>
                                                    <th scope="col" colSpan={8} style={{ fontWeight: 'bold', padding: '10px', border: '1px solid #ddd', textAlign: 'center' }}>{getText('primary_energy_by_region_xaxis', lang)}</th>
                                                ...
                                                <tr>
                                                    {SOURCES_EXCLUDING.map(s => (
                                                        <th key={s} scope="col" style={{ fontWeight: 'bold', padding: '10px', border: '1px solid #ddd' }}>{getText(`primary_energy_production_${s}`, lang)}</th>
                                                    ))}
                                                    <th scope="col" style={{ fontWeight: 'bold', padding: '10px', border: '1px solid #ddd' }}>{lang === 'en' ? 'Total' : 'Total'}</th>
                                                </tr>
                                            </thead>
                                                ...
                                                {PEP_REGIONS.map(region => {
                                                    ...
                                                    return (
                                                        <tr key={region}>
                                                            <th scope="row" style={{ fontWeight: 'bold', padding: '8px', border: '1px solid #ddd' }}>{getText(`primary_energy_by_region_region_${region}`, lang)}</th>
                                                            {vals.map((v, i) => (
                                                                <td key={i} style={{ textAlign: 'center', padding: '8px', border: '1px solid #ddd' }} aria-label={`${getText(`primary_energy_by_region_region_${region}`, lang)}, ${getText(`primary_energy_production_${SOURCES_EXCLUDING[i]}`, lang)}: ${formatProductionByRegionNumber(v, lang)} PJ`}>{formatProductionByRegionNumber(v, lang)}</td>
                                                            ))}
                                                            <td style={{ textAlign: 'center', padding: '8px', border: '1px solid #ddd', fontWeight: 'bold' }}>{formatProductionByRegionNumber(total, lang)}</td>
                                                        </tr>
                                                    );
...
export default PrimaryEnergyByRegion;
