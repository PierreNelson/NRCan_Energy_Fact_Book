...
import { getCleantechCompaniesByIndustryData } from '../utils/dataLoader';
...
const CleantechCompaniesByIndustry = () => {
    ...
    useEffect(() => {
        getCleantechCompaniesByIndustryData()
            .then((data) => {
                setResult(data);
    ...
    const chartTitle = substitute(getText('cleantech_companies_industry_chart_title', lang), { year });
    const fileTitle = `${getText('cleantech_companies_industry_download_title', lang)} ${year}`;
    const industries = selectedRow?.industries?.length ? selectedRow.industries : [];
    const labels = industries.map((item) => getText(`cleantech_companies_industry_industry_${item.key}`, lang));
    ...
    const barTrace = industries.length ? {
        ...
        hovertext: industries.map((item, index) => `<b>${labels[index]}</b><br>${getText('cleantech_companies_industry_col_companies', lang)}: ${formatNumber(item.count)}<br>${getText('cleantech_companies_industry_col_share', lang)}: ${formatPct(item.pct)}`),
        hoverinfo: 'text',
    ...
    const layout = {
        ...
        yaxis: {
            title: { text: getText('cleantech_companies_industry_yaxis', lang), font: axisTitleFont, standoff: 8 },
            range: [0, maxPct],
            tickmode: 'array',
    ...
    const headers = [
        getText('cleantech_companies_industry_col_year', lang),
        getText('cleantech_companies_industry_col_industry', lang),
        getText('cleantech_companies_industry_col_companies', lang),
        getText('cleantech_companies_industry_col_share', lang)
    ];
    const tableRows = industries.map((item) => ({
        year,
        industry: getText(`cleantech_companies_industry_industry_${item.key}`, lang),
        count: item.count,
        pct: item.pct
    }));
    ...
    if (!selectedRow) return <p>{getText('cleantech_companies_industry_no_data', lang)}</p>;

    return (
        <main id="main-content" tabIndex="-1" className="page-content page-63" role="main" aria-labelledby="cleantech-companies-industry-chart-title" style={{ backgroundColor: '#ffffff' }}>
            <style>{`
.page-63 { width: 100%; }
.cleantech-companies-industry-container { width: 100%; padding: 15px 0 0 0; display: flex; flex-direction: column; box-sizing: border-box; flex: 1; overflow: visible; }
.cleantech-companies-industry-chart-frame { background-color: #f5f5f5; padding: 20px; padding-bottom: 20px; border-radius: 8px; margin-top: 0; margin-bottom: 20px; box-sizing: border-box; overflow: visible; }
.cleantech-companies-industry-chart-title { font-family: 'Lato', sans-serif; font-size: 29px; font-weight: bold; color: var(--gc-text); text-align: center; margin: 0 0 5px 0; text-transform: none; }
.cleantech-companies-industry-chart-scroll { width: 100%; overflow: hidden; }
.cleantech-companies-industry-chart { width: 100%; min-width: 0; height: ${layout.height}px; position: relative; z-index: 1; overflow: visible; }
.cleantech-companies-industry-chart > div { width: 100%; height: 100%; overflow: visible; }
.cleantech-companies-industry-chart .js-plotly-plot,
.cleantech-companies-industry-chart .plot-container,
.cleantech-companies-industry-chart .svg-container { overflow: visible !important; }
.cleantech-companies-industry-clear-selection { padding: 6px 12px; background-color: #8C8C8C; border: 1px solid #404040; border-radius: 4px; cursor: pointer; font-family: Arial, sans-serif; font-size: 14px; color: #fff; margin-bottom: 8px; }
.cleantech-companies-industry-table-wrapper { display: block; width: 100%; margin-top: 20px; margin-bottom: 0; }
.cleantech-companies-industry-table-wrapper details > summary { display: block; width: 100%; padding: 12px 15px; background-color: #8C8C8C; border: 1px solid #404040; border-radius: 4px; cursor: pointer; font-family: Arial, sans-serif; font-weight: bold; color: #ffffff; list-style: none; }
.cleantech-companies-industry-table-wrapper details > summary::-webkit-details-marker { display: none; }
.cleantech-companies-industry-table-scrollbar { width: 100%; overflow-x: auto; overflow-y: hidden; margin: 0; }
.cleantech-companies-industry-table-scrollbar > div { height: 20px; }
.cleantech-companies-industry-table-responsive { display: block; width: 100%; overflow-x: auto; border: 1px solid #ddd; background: #fff; scrollbar-width: none; -ms-overflow-style: none; }
.cleantech-companies-industry-table-responsive::-webkit-scrollbar { display: none; }
.cleantech-companies-industry-table-responsive table { width: max-content !important; min-width: 100%; border-collapse: collapse; }
.cleantech-companies-industry-table-responsive th, .cleantech-companies-industry-table-responsive td { white-space: nowrap; padding: 8px 12px; font-family: Arial, sans-serif; color: var(--gc-text); border: 1px solid #ddd; }
.cleantech-companies-industry-download-buttons { display: flex; gap: 10px; margin-top: 15px; flex-wrap: wrap; }
.cleantech-companies-industry-download-buttons button { padding: 8px 16px; border: 1px solid #404040; border-radius: 4px; background: #8C8C8C; cursor: pointer; font-family: Arial, sans-serif; font-weight: bold; color: #ffffff; }
.cleantech-companies-industry-chart-frame button:hover, .cleantech-companies-industry-table-wrapper summary:hover { background-color: #404040 !important; }
@media (max-width: 768px) {
    .cleantech-companies-industry-chart-title { font-size: 26px; }
}
            `}</style>
            <div className="cleantech-companies-industry-container">
                <div className="cleantech-companies-industry-chart-frame">
                    <h2 id="cleantech-companies-industry-chart-title" className="cleantech-companies-industry-chart-title">{chartTitle}</h2>
                    {selectedBars?.length > 0 && <button type="button" className="cleantech-companies-industry-clear-selection" onClick={() => setSelectedBars(null)}>{lang === 'en' ? 'Clear selection' : 'Effacer la sélection'}</button>}
                    <div className="cleantech-companies-industry-chart-scroll">
                        <figure ref={chartRef} className="cleantech-companies-industry-chart" role="region" aria-label={chartTitle} tabIndex="0" style={{ margin: 0 }}>
                            <div aria-hidden="true">
                                {barTrace && (
                                    <Plot
                                        key={`cleantech-companies-industry-${year}-${selectedBars ? selectedBars.join('-') : 'none'}`}
                                        data={[barTrace]}
                                        layout={layout}
                                        config={config}
                    ...
                    <div className="cleantech-companies-industry-table-wrapper">
                        <details className="cleantech-companies-industry-data-table" onToggle={(event) => setIsTableOpen(event.currentTarget.open)}>
                            <summary role="button" aria-expanded={isTableOpen}>
                                <span aria-hidden="true" style={{ marginRight: '8px' }}>{isTableOpen ? '▼' : '▶'}</span>
                                {lang === 'en' ? 'Chart data table' : 'Tableau de données du graphique'}
                                <span className="wb-inv">{lang === 'en' ? ' Press Enter to open or close.' : ' Appuyez sur Entrée pour ouvrir ou fermer.'}</span>
                            </summary>
                            <div ref={tableTopRef} className="cleantech-companies-industry-table-scrollbar" aria-hidden="true"><div /></div>
                            <div ref={tableScrollRef} className="cleantech-companies-industry-table-responsive" role="region" aria-labelledby="cleantech-companies-industry-table-caption" tabIndex={0}>
                                <table className="table table-striped table-hover">
                                    <caption id="cleantech-companies-industry-table-caption" className="wb-inv">{getText('cleantech_companies_industry_table_caption', lang)}</caption>
                                    <thead>
                                        <tr>
                            ...
                            <div ref={tableBottomRef} className="cleantech-companies-industry-table-scrollbar" aria-hidden="true"><div /></div>
                            <div className="cleantech-companies-industry-download-buttons">
...
export default CleantechCompaniesByIndustry;
