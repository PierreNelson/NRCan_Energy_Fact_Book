...
const Page98 = () => {
    ...
    const pageTitle = getText('page98_title', lang);
    const tableCaption = getText('page98_table_caption', lang);
    const fileSlug = getText('page98_download_title', lang);

    const tableHeaders = useMemo(() => [
        getText('page98_col_charger', lang),
        getText('page98_col_input', lang),
        getText('page98_col_outlet', lang),
        getText('page98_col_charging_time', lang),
        getText('page98_col_range', lang),
        getText('page98_col_uses', lang),
    ], [lang]);

    const tableRows = useMemo(() => ROW_KEYS.map((key) => ({
        key,
        charger: getText(`page98_row_${key}_charger`, lang),
        input: INPUT_BY_KEY[key],
        outlet: getText(`page98_row_${key}_outlet`, lang),
        chargingTime: getText(`page98_row_${key}_charging_time`, lang),
        range: getText(`page98_row_${key}_range`, lang),
        uses: getText(`page98_row_${key}_uses`, lang),
    })), [lang]);
...
.page98-inner { width: 100%; padding: 15px 0 40px 0; box-sizing: border-box; }
.page98-title {
...
.page98-table-wrapper {
    width: 100%;
    margin-bottom: 0;
}
.page98-table-wrapper table {
    width: 100% !important;
    min-width: 100% !important;
    max-width: 100% !important;
    table-layout: fixed;
}
.page98-table-wrapper .table-responsive {
    overflow-x: auto;
    width: 100%;
}
.page98-comparison-table {
    width: 100%;
...
.page98-comparison-table th,
.page98-comparison-table td {
...
.page98-comparison-table thead th {
    background-color: #48484A;
    color: #ffffff;
    font-weight: bold;
    text-align: center;
}
.page98-comparison-table thead th .fn-lnk {
    color: var(--gc-text);
    background-color: #ffffff;
    border-color: #6f6f6f;
}
.page98-comparison-table tbody th[scope="row"] {
    background-color: #48484A;
    color: #ffffff;
    font-weight: bold;
    text-align: center;
}
.page98-comparison-table tbody tr:nth-child(odd) td { background-color: #B8BEAD; }
.page98-comparison-table tbody tr:nth-child(even) td { background-color: #fefefe; }
.page98-comparison-table tbody tr td { text-align: left; }
.page98-comparison-table tbody tr:hover td {
    box-shadow: inset 0 0 0 9999px rgba(0, 0, 0, 0.12);
}
.page98-comparison-table tbody tr:hover th[scope="row"] {
    box-shadow: inset 0 0 0 9999px rgba(255, 255, 255, 0.08);
}
.page98-download-buttons { display: flex; gap: 10px; flex-wrap: wrap; margin-top: 15px; }
.page98-download-buttons button:hover { background-color: #404040 !important; }
.page98-footnotes {
...
.page98-footnotes dd { display: flex; align-items: flex-start; gap: 0.75rem; margin-left: 0; margin-bottom: 1rem; }
.page98-footnotes dd p { margin: 0; }
...
@media (max-width: 768px) {
    .page98-title { font-size: 37px; }
    .page98-intro { font-size: 18px; }
}
            `}</style>

            <div className="page98-inner">
                <h1 id="page98-title" className="page98-title">{pageTitle}</h1>

                <p className="page98-intro">
                    {getText('page98_intro_part1', lang)}
                    <strong>{getText('page98_intro_bold1', lang)}</strong>
                    {getText('page98_intro_part2', lang)}
                    <strong>{getText('page98_intro_bold2', lang)}</strong>
                    {getText('page98_intro_part3', lang)}
                    <strong>{getText('page98_intro_bold3', lang)}</strong>
                    {getText('page98_intro_part4', lang)}
                    <strong>{getText('page98_intro_bold4', lang)}</strong>
                    {getText('page98_intro_part5', lang)}
                    <strong>{getText('page98_intro_bold5', lang)}</strong>
                    {getText('page98_intro_part6', lang)}
                </p>

                <div className="page98-table-wrapper">
                    <div className="table-responsive" role="region" aria-label={tableCaption} tabIndex={0}>
                        <table className="page98-comparison-table">
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col">
                                        {tableHeaders[3]}
                                        <span id="fn-asterisk-rf-page98" style={{ verticalAlign: 'super', fontSize: '0.75em' }}>
                                            <a className="fn-lnk" href="#fn-asterisk-page98" onClick={scrollToElement('fn-asterisk-page98')}>
                ...
                <aside className="wb-fnote page98-footnotes" role="note">
                    ...
                    <dl>
                        ...
                        <dd id="fn-asterisk-page98">
                            ...
                            {' '}
                            <p>{getText('page98_footnote', lang)}</p>
                        </dd>
                    </dl>
                </aside>
            </div>
