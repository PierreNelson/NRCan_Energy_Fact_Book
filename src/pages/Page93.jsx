...
const PAGE93_CHART_DATA = buildChartData();
...
const Page93Plot = memo(function Page93Plot({
    plotData, plotLayout, chartConfig, plotHeight, plotKey, onChartClick,
}) {
    return (
        <Plot
            key={plotKey}
...
const Page93 = () => {
    ...
    const ethanolLabel = getText('page93_legend_ethanol', lang);
    const biodieselLabel = getText('page93_legend_biodiesel', lang);
    const chartTitle = substitute(getText('page93_chart_title', lang), textVars);
    const yAxisTitle = getText('page93_yaxis', lang);
    const fileTitle = substitute(getText('page93_download_title', lang), textVars);
    const tableCaption = substitute(getText('page93_table_caption', lang), textVars);

    const tableHeaders = [
        getText('page93_table_col_date', lang),
        `${ethanolLabel} (${yAxisTitle})`,
        `${biodieselLabel} (${yAxisTitle})`,
    ];

    const chartData = PAGE93_CHART_DATA;
    ...
    const plotKey = `page93-${plotHeight}-${useVerticalXTicks ? 'v' : 'h'}`;
...
.page93-inner { width: 100%; padding: 15px 0 40px 0; box-sizing: border-box; }
.page93-title {
...
.page93-chart { width: 100%; min-width: 0; height: ${plotHeight}px; position: relative; }
.page93-chart > div { width: 100%; height: 100%; }
.page93-legend {
    display: flex;
...
.page93-legend-item { display: inline-flex; align-items: center; gap: 8px; white-space: nowrap; }
.page93-legend-line { width: 28px; height: 0; border-top: 3px solid; display: inline-block; }
.page93-download-buttons { display: flex; gap: 10px; flex-wrap: wrap; margin-top: 15px; }
.page93-download-buttons button:hover,
...
.page93-table-scrollbar { width: 100%; overflow-x: auto; overflow-y: hidden; margin: 0; }
.page93-table-scrollbar > div { height: 20px; }
.layout-stacked.page93-content-row { flex-direction: column !important; }
.layout-stacked .page93-text-column,
.layout-stacked .page93-chart-column { width: 100% !important; min-width: 0 !important; }
.layout-stacked .page93-text-column { padding-top: 0; margin-bottom: 24px; }
@media (max-width: 1200px) {
    .page93-content-row { flex-direction: column; }
    .page93-text-column,
    .page93-chart-column { width: 100%; }
    .page93-text-column { padding-top: 0; margin-bottom: 24px; }
}
@media (max-width: 768px) {
    .page93-title { font-size: 37px; }
    .page93-chart-title { font-size: 26px; }
    .page93-bullet { font-size: 18px; }
}
@media (max-width: 480px) {
    .page93-title { font-size: 28px; }
}
            `}</style>
            <div className="page93-inner">
                <h1 id="page93-title" className="page93-title">{getText('page93_title', lang)}</h1>

                <div className={`page93-content-row ${useStackedLayout ? 'layout-stacked' : ''}`}>
                    <aside className="page93-text-column" aria-label={getText('page93_text_aria', lang)}>
                        <ul className="page93-bullet-list">
                            <li className="page93-bullet">
                                {getText('page93_bullet1_part1', lang)}
                                <strong>{getText('page93_bullet1_part2', lang)}</strong>
                            </li>
                            <li className="page93-bullet">
                                {getText('page93_bullet2_part1', lang)}
                                <strong>{getText('page93_bullet2_part2', lang)}</strong>
                                {getText('page93_bullet2_part3', lang)}
                                <strong>{getText('page93_bullet2_part4', lang)}</strong>
                                {getText('page93_bullet2_part5', lang)}
                            </li>
                            <li className="page93-bullet">
                                {getText('page93_bullet3_part1', lang)}
                                <strong>{getText('page93_bullet3_part2', lang)}</strong>
                                {getText('page93_bullet3_part3', lang)}
                            </li>
                            <li className="page93-bullet">
                                {getText('page93_bullet4_part1', lang)}
                                <strong>{getText('page93_bullet4_part2', lang)}</strong>
                                {getText('page93_bullet4_part3', lang)}
                            </li>
                        </ul>
                    </aside>

                    <div className="page93-chart-column">
                        <div className="page93-chart-frame">
                            <h2 id="page93-chart-title" className="page93-chart-title">{chartTitle}</h2>
                            ...
                            <figure
                                ref={chartRef}
                                className="page93-chart"
                                role="region"
                                aria-label={getText('page93_chart_aria', lang)}
                                tabIndex={0}
                                style={{ margin: 0 }}
                            >
                                <Page93Plot
                                    ...
                                    chartConfig={{
                                        ...
                                        modeBarButtonsToAdd: [{
                                            name: getText('page93_download_png', lang),
                                            icon: {
                                                width: 24,
                                                height: 24,
                            ...
                            <div className="page93-legend" aria-hidden="true">
                                <span className="page93-legend-item">
                                    <span className="page93-legend-line" style={{ borderColor: COLORS.biodiesel }} />
                                    {biodieselLabel}
                                </span>
                                <span className="page93-legend-item">
                                    <span className="page93-legend-line" style={{ borderColor: COLORS.ethanol }} />
                                    {ethanolLabel}
                                </span>
                            </div>
                            ...
                            <details className="data-table-wrapper" onToggle={(e) => setIsTableOpen(e.currentTarget.open)}>
                                ...
                                <div ref={topScrollRef} className="page93-table-scrollbar" aria-hidden="true"><div /></div>
                                <div
                                    ref={tableScrollRef}
                                ...
                                <div ref={bottomScrollRef} className="page93-table-scrollbar" aria-hidden="true"><div /></div>
                                <div className="page93-download-buttons">
                                    <button type="button" onClick={downloadCsv} style={downloadBtnStyle}>
                                        {getText('page93_download_csv', lang)}
                                    </button>
                                    <button type="button" onClick={downloadDocx} style={downloadBtnStyle}>
                                        {getText('page93_download_docx', lang)}
                                    </button>
                                </div>
                            </details>
                        </div>
