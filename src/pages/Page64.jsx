...
const Page64 = () => {
    ...
    const productionGraphicTitle = `${stripHtml(getText('page64_production_label', lang))} \u2013 ${formatTwh(production.total)} ${getText('page64_unit', lang)} (${year})`;
    const exportsGraphicTitle = `${stripHtml(getText('page64_exports_label', lang))} \u2013 ${formatTwh(exportsBlock.total)} ${getText('page64_unit', lang)} (${year})`;
    ...
    const downloadProductionCSV = () => {
        ...
        saveAs(new Blob([csvContent], { type: 'text/csv;charset=utf-8;' }), downloadFilename('page64_production_csv_slug'));
    };

    const downloadProductionDocx = async () => {
        const title = stripHtml(getText('page64_production_label', lang));
        const headers = [
            getText('page64_table_col_year', lang),
            getText('page64_table_col_total', lang),
            ...PRODUCTION_COLUMN_KEYS.map((k) => `${stripHtml(page64CountryLabel(k, lang))} (%)`),
        ...
        saveAs(await Packer.toBlob(doc), downloadFilename('page64_production_docx_slug'));
    };
    ...
    const downloadExportsCSV = () => {
        ...
        const csvContent = [headers.join(','), ...rows.map((r) => r.join(','))].join('\n');
        saveAs(new Blob([csvContent], { type: 'text/csv;charset=utf-8;' }), downloadFilename('page64_exports_csv_slug'));
    };

    const downloadExportsDocx = async () => {
        const title = stripHtml(getText('page64_exports_label', lang));
        const headers = [
            getText('page64_table_col_year', lang),
            getText('page64_table_col_total', lang),
            ...EXPORT_COLUMN_KEYS.map((k) => `${stripHtml(page64CountryLabel(k, lang))} (%)`),
        ...
        saveAs(await Packer.toBlob(doc), downloadFilename('page64_exports_docx_slug'));
    };
    ...
    const downloadProductionPng = () => {
        ...
        exportCapsuleInfographicPng({
            ...
            rows,
            filename: downloadFilename('page64_production_png_slug', year),
        });
    };
    ...
    const downloadExportsPng = () => {
        exportCapsuleInfographicPng({
            ...
            filename: downloadFilename('page64_exports_png_slug', year),
        });
    };
    ...
    const renderCapsuleRow = (row, maxPct) => {
        ...
        return (
            <div key={`${row.rank}-${row.name}`} className={`page64-capsule-row ${isCanada ? 'page64-capsule-row-canada' : ''}`}>
                <div className="page64-capsule-bar-wrap">
                    <div className="page64-capsule-bar" style={{ width: `${barWidthPct}%` }}>
                        <span className="page64-capsule-bar-text">{row.rank} {row.name}</span>
                    </div>
                </div>
                <div className="page64-capsule-connector" />
                <div className="page64-capsule-pill">
                    <span className="page64-capsule-pill-text">{row.pctRounded}%</span>
                </div>
            </div>
        );
    };
    ...
    const renderProductionCapsules = () => {
        ...
        return (
            <>
                ...
                <div className="page64-capsule-row page64-capsule-ellipsis" aria-hidden="true"><span>...</span></div>
                {renderCapsuleRow(
                    {
                        rank: production.canada.rank,
                        name: page64CountryLabel('canada', lang),
    ...
    const renderDataTable = ({
        ...
        <details ref={detailsRef} className="data-table-wrapper" onToggle={(e) => setIsOpen(e.currentTarget.open)}>
            ...
            <div ref={topRef} className="page64-table-scrollbar" aria-hidden="true"><div /></div>
            ...
            <div ref={bottomRef} className="page64-table-scrollbar" aria-hidden="true"><div /></div>
    ...
    return (
        <main id="main-content" tabIndex="-1" className="page-content page-64" role="main" aria-labelledby="page64-main-title" style={{ backgroundColor: '#ffffff' }}>
            <style>{`
...
.page64-container { width: 100%; padding: 15px 0 40px 0; display: flex; flex-direction: column; box-sizing: border-box; }
.page64-title { font-family: 'Lato', sans-serif; font-size: 41px; font-weight: bold; color: var(--gc-text); margin-top: 0; margin-bottom: 25px; position: relative; padding-bottom: 0.5em; text-transform: none; }
.page64-title::after { content: ''; position: absolute; left: 0; bottom: 0.2em; width: 72px; height: 6px; background-color: var(--gc-red); }
.page-64 p.page64-subtitle { font-family: 'Lato', sans-serif; font-size: 39px; font-weight: bold; color: var(--gc-text); margin: 0 0 15px 0; text-transform: none; }
.page64-content-wrapper { display: flex; flex-direction: column; gap: 30px; margin-top: 20px; }
.page64-chart-section { width: 100%; display: flex; flex-direction: column; align-items: flex-start; }
.page64-graphic-title { font-family: 'Lato', sans-serif; font-size: 29px; font-weight: bold; color: #000000; margin: 0 0 10px 0; text-align: left; width: 100%; text-transform: none; }
.page64-chart-container { position: relative; width: 100%; max-width: 1280px; margin-left: auto; margin-right: auto; }
.page64-capsule-graphic { display: flex; flex-direction: column; gap: 8px; width: 100%; font-family: 'Noto Sans', sans-serif; }
.page64-capsule-row { display: flex; align-items: center; min-height: 52px; gap: 0; min-width: 0; }
.page64-capsule-bar-wrap { flex: 0 0 28%; min-width: 0; display: flex; align-items: center; }
.page64-capsule-bar { min-height: 44px; min-width: 82px; border-radius: 999px; background-color: #d1d1d1; display: flex; align-items: center; justify-content: center; text-align: center; padding: 10px 12px; font-size: clamp(13px, 1.5vw, 15px); color: #333; box-sizing: border-box; width: 100%; max-width: 100%; }
.page64-capsule-bar-text { word-wrap: break-word; overflow-wrap: break-word; word-break: break-word; white-space: normal; line-height: 1.3; }
.page64-capsule-row-canada .page64-capsule-bar { background-color: ${CANADA_CAPSULE}; color: #fff; font-weight: bold; }
.page64-capsule-connector { flex: 1 1 0; min-width: 10px; height: 2px; background-color: #b0b0b0; }
.page64-capsule-row-canada .page64-capsule-connector { background-color: ${CANADA_CAPSULE}; }
.page64-capsule-pill { flex: 0 0 64px; height: 36px; min-width: 64px; border-radius: 999px; border: 1px solid #b0b0b0; background-color: #fff; display: flex; align-items: center; justify-content: center; font-size: 15px; color: #333; box-sizing: border-box; }
.page64-capsule-row-canada .page64-capsule-pill { border-width: 2px; border-color: ${CANADA_CAPSULE}; font-weight: bold; font-size: 16px; }
.page64-capsule-ellipsis { min-height: 28px; padding-left: 4%; color: #333; font-size: 18px; font-weight: bold; }
.page64-download-buttons { display: flex; gap: 10px; flex-wrap: wrap; margin-top: 15px; }
.page64-chart-section .data-table-wrapper { padding-top: 5px; margin-top: 20px; width: 100%; box-sizing: border-box; }
...
.page64-table-scrollbar { width: 100%; overflow-x: auto; overflow-y: hidden; margin: 0; }
.page64-table-scrollbar > div { height: 20px; }
@media (max-width: 768px) {
    .page64-title { font-size: 37px; }
    .page-64 p.page64-subtitle { font-size: 35px; }
    .page64-graphic-title { font-size: 26px; }
}
            `}</style>

            <div className="page64-container">
                <header>
                    <h1 id="page64-main-title" className="page64-title">{getText('page64_title', lang)}</h1>
                    <p className="page64-subtitle">{getText('page64_subtitle', lang)}</p>
                </header>
                ...
                <div className="page64-content-wrapper layout-stacked">
                    <div className="page64-chart-section">
                        <h3 className="page64-graphic-title">{productionGraphicTitle}</h3>
                        <div className="page64-chart-container" role="region" aria-label={productionAria} tabIndex="0">
                            <div className="page64-capsule-graphic" aria-hidden="true">{renderProductionCapsules()}</div>
                        </div>
                        <div className="page64-download-buttons">
                            <button type="button" onClick={downloadProductionPng} style={downloadBtnStyle}>{getText('page64_download_png', lang)}</button>
                        </div>
                        {renderDataTable({
                            ...
                            captionKey: 'page64_table_caption_production',
                    ...
                    <div className="page64-chart-section">
                        <h3 className="page64-graphic-title">{exportsGraphicTitle}</h3>
                        <div className="page64-chart-container" role="region" aria-label={exportsAria} tabIndex="0">
                            <div className="page64-capsule-graphic" aria-hidden="true">{renderExportCapsules()}</div>
                        </div>
                        <div className="page64-download-buttons">
                            <button type="button" onClick={downloadExportsPng} style={downloadBtnStyle}>{getText('page64_download_png', lang)}</button>
                        </div>
                        {renderDataTable({
                            ...
                            captionKey: 'page64_table_caption_exports',
