...
**Symphony Windows:** there is no in-process `linear_graphql` RPC tool. Use the **bundled CLI** instead (same auth as Symphony):

- Environment: `LINEAR_API_KEY`, absolute script path in `SYMPHONY_LINEAR_GRAPHQL_CLI`.
- Run: `python` + that **file path** + JSON on stdin (see prompt *Symphony bundled Linear GraphQL*). From the issue clone, **do not** use `python -m symphony_windows.linear_graphql_cli` unless the package is installed.

**Elixir / other Symphony builds** may expose a true `linear_graphql` client tool. When present, use it as below.

Tool input (equivalent shape for the CLI stdin):
